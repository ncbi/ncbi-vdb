...
//TODO check out of space condition
    ...
    /** Created a new Manager that will be able to [down]load the system library
        specified by the libname argument. */
    private LibManager(String libname)
    {
        this(libname, null);
    }
    ...
    private LibManager(String libname,
        Location locations[])
    {
        Logger.fine("Creating LibManager(" + libname + ")...");
        ...
        if (System.getProperty("vdb.deleteLibraries") != null) {
            ...
            LibPathIterator.deleteLibraries(this);
        }
    }
   ...
   when we were not able to System.load Library it. */
    ...
    private boolean download()
    {
        ...
        for (int i = 0; i < SRATOOLKIT_CGI.length; ++i) {
            int code = HttpManager.post(SRATOOLKIT_CGI[i], request, this);
            if (code == 200) {
                return true;
            } else {

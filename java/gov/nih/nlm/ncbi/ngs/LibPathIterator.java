...
class LibPathIterator {
    ...
    private static final String NCBI_NGS_JAR_NAME = "ncbi-ngs.jar";
    ...
    static boolean list()
    {
        return iterate(new LibManager(), false);
    }


    /** delete all libraries found by the LibManager */
    static boolean deleteLibraries(LibManager mgr)
    {
        return iterate(mgr, true);
    }
...
Here we use it just to find where to write the downoaded file. */
    ...
    private static boolean iterate(LibManager mgr, boolean rm)
    {
...
//      String filename[] = LibManager.mapLibraryName("ncbi-ngs-jni");
        LibPathIterator it =
            new LibPathIterator(mgr, LibManager.mapLibraryName(mgr.getName()));
        String s = null;

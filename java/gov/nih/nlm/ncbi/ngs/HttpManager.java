...
class HttpManager
{
    ...
    private static boolean get(String from,
        FileCreator creator)
    {
        System.err.println(from + " ->" + creator.getName() + "...");
    ...
    static int post(String spec,
        ...
        FileCreator creator)
    {
        Logger.fine(spec + "?" + request + " -> " + creator.getName() + "...");
    ...
    private static boolean download(InputStream in,
        FileCreator creator)
    {
        java.io.BufferedOutputStream bout = creator.create();
        if (bout == null) {

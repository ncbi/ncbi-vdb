...
def to_bytes( s ) :
    return str.encode( s ) if type( s ) == str else s

def to_char_p( s ) :
    return c_char_p( str.encode( s ) if type( s ) == str else s )
...
class KNamelist :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KNamelistRelease()", self )

    def count( self ) :
        n = c_int()
        rc = self.__mgr.KNamelistCount( self.__ptr, byref( n ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "KNamelistCount()", self )
        return n.value
    ...
    def to_list( self ) :
        ...
        for idx in range( 0, self.count() ) :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "KNamelistGet( %d )"%( idx ), self )
            if PY3 :
...
class VColumn :
    ...
    def _update( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorDatatype( '%s.%s' )"%( self.tabname, self.name ), self )
        ( dict, dflt ) = type_xf[ self.__tdes.domain ]
        self.column_type = dict[ self.__tdes.bits ]
        if self.column_type == None :
            self.column_type == dflt

    def cursor( self ) : return self.__cur
    def domain( self ) : return TypeDomain( self.__tdes.domain )
    def bits( self ) :  return self.__tdes.bits
    def dim( self ) : return self.__tdes.dim
    ...
    def read( self, row ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorCellDataDirect( '%s.%s', #%d )"%( self.tabname, self.name, row ), self )
    ...
    def __write_values( self, data ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorWrite( %s.%s )"%( self.tabname, self.name ), self )
    ...
    def __write_string( self, data ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorWrite( %s.%s, %s )"%( self.tabname, self.name, data ), self )
    ...
    def set_default( self, data ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorDefault( %s.%s )"%( self.tabname, self.name ), self )
    ...
    def default_string( self, data ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorDefault( %s.%s, %s )"%( self.tabname, self.name, data ), self )
    ...
    def row_range( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorIdRange( '%s.%s' )"%( self.tabname, self.name ), self )
    ...
    def next_row( self, current_row ) :
        ...
        return None if rc != 0 else res.value
...
class ReferenceObj :
    ...
    def Idx( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_Idx()", self )
        return res.value
    ...
    def IdRange( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_IdRange()", self )
        return ( start.value, stop.value )

    def Bin( self ) :
        res = c_int()
        rc = self.__mgr.ReferenceObj_Bin( self.__ptr, byref( res ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_Bin()", self )
        return res.value

    def SeqId( self ) :
        res = c_char_p()
        rc = self.__mgr.ReferenceObj_SeqId( self.__ptr, byref( res ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_SeqId()", self )
        return res.value

    def Name( self ) :
        res = c_char_p()
        rc = self.__mgr.ReferenceObj_Name( self.__ptr, byref( res ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_Name()", self )
        return res.value

    def SeqLength( self ) :
        res = c_int()
        rc = self.__mgr.ReferenceObj_SeqLength( self.__ptr, byref( res ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_SeqLength()", self )
        return res.value

    def Circular( self ) :
        res = c_bool()
        rc = self.__mgr.ReferenceObj_Circular( self.__ptr, byref( res ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_Circular()", self )
        return res.value

    def External( self ) :
        res = c_bool()
        rc = self.__mgr.ReferenceObj_External( self.__ptr, byref( res ), c_void_p( 0 ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_External()", self )
        return res.value
    ...
    def Read( self, offs, len ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_Read( %d.%d )" % ( offs, len ), self )
        if PY3 : return buffer.value.decode( "utf-8" )
        return buffer.value

    def GetIdCount( self, row_id ) :
        res = c_int()
        rc = self.__mgr.ReferenceObj_GetIdCount( self.__ptr, c_longlong( row_id ), byref( res ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceObj_GetIdCount( %d )" % row_id, self )
        return res.value
...
class ReferenceList :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceList_Release()", self )

    def count( self ) :
        res = c_int()
        rc = self.__mgr.ReferenceList_Count( self.__ptr, byref( res ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceList_Count()", self )
        return res.value

    def find( self, name ) :
        ptr = c_void_p()
        rc = self.__mgr.ReferenceList_Find( self.__ptr, byref( ptr ), to_char_p( name ), c_int( len( name ) ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceList_Find( '%s' )" % name, self )
        return ReferenceObj( self.__mgr, ptr )

    def get( self, idx ) :
        ptr = c_void_p()
        rc = self.mgr.ReferenceList_Get( self.__ptr, byref( ptr ), c_int( idx ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceList_Get( %d )" % idx, self )
        return ReferenceObj( self.__mgr, ptr )
...
class VCursor :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorRelease( '%s' )"%( self.__tab._VTable__name ), self )
    ...
    def AddColumn( self, name ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorAddColumn( %s.%s )"%( self.__tab.name, name ), self )
        return VColumn( self.__mgr, self, idx.value, name, self.__tab._VTable__name )

    def Open( self ) :
        rc = self.__mgr.VCursorOpen( self.__ptr )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorOpen( '%s' )"%( self.__tab._VTable__name ), self )

    def Commit( self ) :
        rc = self.__mgr.VCursorCommit( self.__ptr )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorCommit( %s )"%( self.__tab._VTable__name ), self )

    def OpenRow( self ) :
        rc = self.__mgr.VCursorOpenRow( self.__ptr )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorOpenRow( '%s' )"%( self.__tab._VTable__name ), self )

    def CommitRow( self ) :
        rc = self.__mgr.VCursorCommitRow( self.__ptr )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorCommitRow( '%s' )"%( self.__tab._VTable__name ), self )

    def RepeatRow( self, count ) :
        rc = self.__mgr.VCursorRepeatRow( self.__ptr, count )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorRepeatRow( '%s', %d )"%( self.__tab._VTable__name, count ), self )

    def CloseRow( self ) :
        rc = self.__mgr.VCursorCloseRow( self.__ptr )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorCloseRow( '%s' )"%( self.__tab._VTable__name ), self )

    def FlushPage( self ) :
        rc = self.__mgr.VCursorFlushPage( self.__ptr )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorFlushPage( '%s' )"%( self.__tab._VTable__name ), self )

    def RowId( self ) :
        row_id = c_longlong()
        rc = self.__mgr.VCursorRowId( self.__ptr, byref( row_id ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorRowId( '%s' )"%( self.__tab._VTable__name ), self )
        return row_id.value

    def SetRowId( self, value ) :
        rc = self.__mgr.VCursorSetRowId( self.__ptr, value )
        if rc != 0 : self.__mgr.raise_rc( rc, "VCursorSetRowId( '%s', %d )"%( self.__tab._VTable__name, value ), self )
    ...
    def ReferenceList( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceList_MakeCursor()", self )
...
class KIndex :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexRelease( '%s' )"%( self.__name ), self )

    def Version( self ) :
        vers = c_int()
        rc = self.__mgr.KIndexVersion( self.__ptr, byref( vers ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexVersion( '%s' )"%( self.__name ), self )
        return version( vers.value )

    def Type( self ) :
        type = c_int()
        rc = self.__mgr.KIndexType( self.__ptr, byref( type ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexType( '%s' )"%( self.__name ), self )
    ...
    def FindText( self, key ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexFindText( %s, %s )"%( self.__name, key ), self )
    ...
    def ProjectText( self, row_id, bufsize = 1024 ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexProjectText( %s, %d )"%( self.__name, row_id ), self )
        if PY3 : return ( buffer.value.decode( "utf-8" ), start_id.value, id_count.value )
        return ( buffer.value, start_id.value, id_count.value )

    def Commit( self ) :
        rc = self.__mgr.KIndexCommit( self.__ptr )
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexCommit( %s )"%( self.__name ), self )
    ...
    def InsertText( self, unique, key, value ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexInsertText( %s, k=%s, v=%d )"%( self.__name, key, value ), self )

    def DeleteText( self, key ) :
        rc = self.__mgr.KIndexDeleteText( self.__ptr, to_char_p( key ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexDeleteText( %s, k=%s )"%( self.__name, key ), self )
    ...
    def InsertU64( self, unique, key_start, keylen, value_start, value_len ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexInsertU64( %s, k=%d.%d, v=%d.%d )"%( self.__name, key_start, key_len, value_start, value_len ), self )

    def DeleteU64( self, key ) :
        rc = self.__mgr.KIndexDeleteU64( self.__ptr, c_longlong( key ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "KIndexDeleteU64( %s, k=%d )"%( self.__name, key ), self )
...
class KMDataNode :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeRelease()", self )
    ...
    def OpenNode( self, path, open_mode = OpenMode.Read ) :
        ...
        if open_mode == OpenMode.Write :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeOpenNodeUpdate( %s )"%( path ), self )
        else :
            rc = self.__mgr.KMDataNodeOpenNodeRead( self.__ptr, byref( node ), to_char_p( path ) )
            if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeOpenNodeRead( %s )"%( path ), self )
        return KMDataNode( self.__mgr, node )

    def ByteOrder( self ) :
        order = c_bool()
        rc = self.__mgr.KMDataNodeByteOrder( self.__ptr, byref( order ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeByteOrder()", self )
        return order.value

    def ListChildren( self ) :
        l = c_void_p()
        rc = self.__mgr.KMDataNodeListChildren( self.__ptr, byref( l ) )
        if rc == 0 : return KNamelist( self.__mgr, l ).to_list()
        return []
    ...
    def size( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeRead()", self )
        return num_read.value + remaining.value

    def to_read( self, req_len ) :
        ns = self.size()
        if req_len > 0 : return min( req_len, ns )
        return ns
    ...
    def Read( self, buffer_size, to_read, offset ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeRead( offset = %d )"%( offset ), self )
        if PY3 :
    ...
    def write_string( self, data, append = False ) :
        ...
        if append :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeAppend()", self )
        else :
            rc = self.__mgr.KMDataNodeWrite( self.__ptr, p, c_size_t( len( data ) ) )
            if rc != 0 : self.__mgr.raise_rc( rc, "KMDataNodeWrite()", self )
...
class KMetadata :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KMetadataRelease()", self )
    ...
    def OpenNode( self, path, open_mode = OpenMode.Read ) :
        ...
        if open_mode == OpenMode.Write :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "KMetadataOpenNodeUpdate( %s )"%( path ), self )
        else :
            rc = self.__mgr.KMetadataOpenNodeRead( self.__ptr, byref( ptr ), to_char_p( path ) )
            if rc != 0 : self.__mgr.raise_rc( rc, "KMetadataOpenNodeRead( %s )"%( path ), self )
...
class VTable :
    ...
    def __del__( self ) :
        ...
        if self.__kdb_ptr != None :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "KTableRelease( %s )"%( self.__name ), self )
    ...
    def ListCol( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VTableListCol( %s )"%( self.__name ), self )
    ...
    def __OpenKTableRead__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VTableOpenKTableRead( '%s' )"%( self.__name ), self )
        self.__kdb_ptr = k_table

    def __OpenKTableUpdate__( self ) :
        k_table = c_void_p()
        rc = self.__mgr.VTableOpenKTableUpdate( self.__ptr, byref( k_table ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VTableOpenKTableUpdate( '%s' )"%( self.__name ), self )
        self.__kdb_ptr = k_table
    ...
    def OpenIndexRead( self, name ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KTableOpenIndexRead( '%s' )"%( self.__name ), self )
    ...
    def CreateIndex( self, name, idx_type = IndexType.Text, create_mode = CreateMode.Init ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KTableCreateIndex( '%s' )"%( self.__name ), self )
        return KIndex( self.__mgr, k_idx, name )
    ...
    def OpenMetadata( self, open_mode = OpenMode.Read ) :
        ...
        if open_mode == OpenMode.Write :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "VTableOpenMetadataUpdate( '%s' )"%( self.__name ), self )
        else :
            if self.__kdb_ptr == None :
                self.__OpenKTableRead__()
            rc = self.__mgr.KTableOpenMetadataRead( self.__kdb_ptr, byref( k_meta ) )
            if rc != 0 : self.__mgr.raise_rc( rc, "KTableOpenMetadataRead( '%s' )"%( self.__name ), self )
        return KMetadata( self.__mgr, k_meta )
    ...
    def CreateCursor( self, open_mode = OpenMode.Read, ins_mode = CursorMode.Insert ) :
        ...
        if open_mode == OpenMode.Write :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "VTableCreateCursorWrite( %s )"%( self.__name ), self )
        else :
            rc = self.__mgr.VTableCreateCursorRead( self.__ptr, byref( c ) )
            if rc != 0 : self.__mgr.raise_rc( rc, "VTableCreateCursorRead( %s )"%( self.__name ), self )
    ...
    def CreateCachedCursorRead( self, cache_size ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VTableCreateCachedCursorRead( %s, %d )"%( self.__name, cache_size ), self )
        return VCursor( self, c )
    ...
    def ReferenceList( self, options = 2 ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceList_MakeTable( '%s' )"%( self.__name ), self )
...
class VDatabase :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseRelease( %s )"%( self.__name ), self )
    ...
    def ListTbl( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseListTbl( %s )"%( self.__name ), self )
        return KNamelist( self.__mgr, l ).to_list()

    def ListDB( self ) :
        l = c_void_p()
        rc = self.__mgr.VDatabaseListDB( self.__ptr, byref( l ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseListDB( %s )"%( self.__name ), self )
        return KNamelist( self.__mgr, l ).to_list()
    ...
    def OpenTable( self, name, mode = OpenMode.Read ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseOpenTable( %s.%s )"%( self.__name, name ), self )
    ...
    def OpenDB( self, name, mode = OpenMode.Read ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseOpenDB( %s.%s )"%( self.__name, name ), self )
    ...
    def CreateTable( self, spec, name = None, mode = CreateMode.Init ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseCreateTable( %s.%s )"%( self.__name, spec ), self )
    ...
    def CreateDB( self, spec, name = None, mode = CreateMode.Init ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseCreateDB( %s.%s )"%( self.__name, spec ), self )
        return VDatabase( self.__mgr, new_db, p.value )

    def DropTable( self, name ) :
        rc = self.__mgr.VDatabaseDropTable( self.__ptr, to_char_p( name ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseDropTable( %s.%s )"%( self.__name, name ), self )

    def DropDB( self, name ) :
        rc = self.__mgr.VDatabaseDropDB( self.__ptr, to_char_p( name ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseDropDB( %s.%s )"%( self.__name, name ), self )

    def Lock( self, name, what ) :
        rc = self.__mgr.VDatabaseLock( self.__ptr, c_int( what ), to_char_p( name ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseLock( %s.%s )"%( self.__name, name ), self )

    def UnLock( self, name, what ) :
        rc = self.__mgr.VDatabaseUnlock( self.__ptr, c_int( what ), to_char_p( name ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VDatabaseUnlock( %s.%s )"%( self.__name, name ), self )
    ...
    def ReferenceList( self, options = 2 ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "ReferenceList_MakeDatabase( %s )"%( self.__name ), self )
        return ReferenceList( self.__mgr, ptr )
...
class VSchema :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VSchemaRelease()", self )
    ...
    def ParseText( self, schema_txt ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VSchemaParseText()", self )

    def ParseFile( self, schema_file ) :
        rc = self.__mgr.VSchemaParseFile( self.__ptr, to_char_p( schema_file ) )
        if rc != 0 : self.mgr.raise_rc( rc, "VSchemaParseFile( '%s' )"%schema_file, self )

    def AddIncludePath( self, path ) :
        rc = self.__mgr.VSchemaAddIncludePath( self.__ptr, to_char_p( path ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VSchemaAddIncludePath( '%s' )"%path, self )
...
class KRepository :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KRepositoryRelease()", self )
    ...
    def AddRef ( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KRepositoryAddRef()", self )
    ...
    def Name( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KRepositoryName()", self )
        if PY3 : return p.value.decode( "utf-8" )
        return p.value
    ...
    def DisplayName( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KRepositoryDisplayName()", self )
        if PY3 : return p.value.decode( "utf-8" )
        return p.value
    ...
    def SetRoot( self, root ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KRepositorySetRoot( '%s' )"%( root ), self )
    ...
    def SetDisabled( self, value ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KRepositorySetDisabled()", self )
...
class KRepositoryMgr :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KRepositoryMgrRelease()", self )
...
class KConfig :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KConfigRelease()", self )
    ...
    def ReadString( self, path ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "KConfigReadString( '%s' )"%( path ), self )
    ...
    def MakeRepositoryMgr( self, mode = OpenMode.Read ) :
        ...
        if mode == OpenMode.Read :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "KConfigMakeRepositoryMgrRead()", self )
        else :
            rc = self.__mgr.KConfigMakeRepositoryMgrUpdate( self.__ptr, byref( ptr ) )
            if rc != 0 : self.__mgr.raise_rc( rc, "KConfigMakeRepositoryMgrUpdate()", self )
...
class VPath :
    ...
    def __str__( self ) :
        if self.__ptr :
            ...
            if rc != 0 : self.__mgr.raise_rc( rc, "VPathMakeString()", self )
            res = sp.contents.addr
            self.__mgr.string_whack( sp.contents )
        else :
            res = ""
...
class VResolver :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VResolverRelease()", self )
    ...
    def QueryLocal( self, path ) :
        ...
        if rc != 0 : return vpath( self.__mgr, 0 )
    ...
    def RemoteEnable( self, state = ResolvEnable.UseConfig ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VResolverRemoteEnable()", self )
...
class VFSManager :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VFSManagerRelease()", self )
    ...
    def GetResolver( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VFSManagerGetResolver()", self )
    ...
    def MakeResolver( self, cfg ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "VFSManagerMakeResolver()", self )
        return VResolver( self.__mgr, ptr )

    def MakePath( self, s_path ) :
        path = c_void_p()
        rc = self.__mgr.VFSManagerMakePath( self.__ptr, byref( path ), to_char_p( s_path ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "VFSManagerMakePath( '%s' )"%( s_paht ), self )
...
class RefVariation :
    ...
    def __del__( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "RefVariationRelease()", self )
    ...
    def GetIUPACSearchQuery( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "RefVariationGetIUPACSearchQuery()", self )
    ...
    def GetSearchQueryLenOnRef( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "RefVariationGetSearchQueryLenOnRef()", self )
        return len.value
    ...
    def GetAllele( self ) :
        ...
        if rc != 0 : self.__mgr.raise_rc( rc, "RefVariationGetAllele()", self )
        return ( txt.value, len.value, start.value )

    def GetAlleleLenOnRef( self ) :
        len = c_int()
        rc = self.__mgr.RefVariationGetAlleleLenOnRef( self.__ptr, byref( len ) )
        if rc != 0 : self.__mgr.raise_rc( rc, "RefVariationGetAlleleLenOnRef()", self )
        return len.value


#------------------------------------------------------------------------------------------------------------
def mode_and_platform_to_libname( mode ) :
    libname = None
    pf = platform.system()
    if pf == "Windows":
...
def platform_to_path_seg() :
    pf = platform.system()
    if pf == "Windows" : return "win"
    elif pf == "Darwin" : return "mac"
    elif pf == "Linux" : return "linux"
    return None

def check_path_leaf( path, leaf ) :
    if path == None : return None
    if leaf == None : return None
    res = os.path.join( os.path.sep, path, leaf  )
    return res if os.path.isdir( res ) else None

def check_path_leaf_opt( path, leaf1, leaf2 ) :
    res = check_path_leaf( path, leaf1 )
    return check_path_leaf( path, leaf2 ) if res == None else res

def check_ncbi_lib64( libname ) :
    if libname == None : return None
    path = os.path.join( os.path.sep, os.path.expanduser( "~" ), ".ncbi", "lib64", libname )
    return path if os.path.isfile( path ) else None

#example: $HOME/ncbi-outdir/ncbi-vdb/linux/gcc/x86_64/dbg/lib
def check_ncbi_outdir( libname ) :
    if libname == None : return None
    path = check_path_leaf( os.path.expanduser( "~" ), "ncbi-outdir" )
    path = check_path_leaf( path, "ncbi-vdb" )
    path = check_path_leaf( path, platform_to_path_seg() )
    path = check_path_leaf_opt( path, "gcc", "clang" )
    path = check_path_leaf( path, platform.machine() )
    path = check_path_leaf_opt( path, "rel", "dbg" )
    path = check_path_leaf( path, "lib" )
    if path == None : return None
    path = os.path.join( os.path.sep, path, libname )
    return path if os.path.isfile( path ) else None

def check_usr_local( libname ) :
    if libname == None : return None
    path = os.path.join( os.path.sep, "usr", "local", "ncbi", "ncbi-vdb", "lib", libname )
    return path if os.path.isfile( path ) else None

def find_lib( libname ) :
    if libname is None : return None
    if os.path.isfile( libname ) : return libname  #found in the current directory...
    #path = check_ncbi_lib64( libname ) #looking in $HOME/.ncbi/lib64
    #if path != None : return path
    path = check_ncbi_outdir( libname ) #looking in $HOME/ncbi-outdir/ncbi-vdb/linux/gcc/x86_64/dbg/lib
    if path != None : return path
    return check_usr_local( libname )
...
class manager :
    def __init__( self, mode = OpenMode.Read, path = None ) :
        ...
        self.libname = find_lib( mode_and_platform_to_libname( mode ) )
        if self.libname == None :
            raise vdb_error( 0, "cannot find library", None )
        try :
            self.__lib = cdll.LoadLibrary( self.libname )
        except :
            raise vdb_error( 0, "cannot load library '%s'"%self.libname, None )

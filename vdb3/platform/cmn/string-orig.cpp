...
namespace cmn
{
    ...
    /* UTF8
     *  by all accounts, characters should be natural number codes
     *  but it is common in UTF8 processing to make use of the fact
     *  that most compilers treat "char" as "signed char" - although
     *  some don't!
     *
     *  it is convenient to use signed byte relational operators when
     *  walking across UTF8 bytes, so we treat them as signed wherever
     *  this is significant.
     */
     ...
     */
    static
    bool is_ascii ( const signed char * zstr )
    {
    ...
    {
        return is_ascii ( ( const signed char * ) zstr );
    }

    static
    bool is_ascii ( const signed char * str, size_t bytes )
    {
        // a null pointer isn't anything
        if ( str == nullptr )
            return false;
    ...
    {
        return is_ascii ( ( const signed char * ) str, bytes );
    }
     ...
     */
    static
    long int scan_fwd ( const signed char * data, size_t bytes, size_t offset )
    {
     ...
     */
    static
    long int scan_rev ( const signed char * data, size_t bytes, size_t offset )
    {
        if ( offset <= 0 || offset > bytes )
            return 1;
     ...
     */
    static
    count_t string_length ( const signed char * data, size_t bytes )
    {
        // allow empty string
        count_t cnt = 0;
    ...
    {
        return string_length ( ( const signed char * ) data, bytes );
    }
     ...
     */
    static
    UTF32 utf8_to_utf32 ( const signed char * data, size_t bytes, size_t offset )
    {
...
#define STRING_RFIND_UTF8_RETURN_1( meth, str, query )  \
        ...
        : cnt ( 0 )
    {
        ...
        if ( is_ascii ( zstr ) )
        {
            ...
            std :: cerr
                << __func__
                << " constructing from NUL-terminated ASCII string: '"
                << zstr
                << "'\n"
                ;
#endif
        ...
        else
        {
            ...
            std :: cerr
                << __func__
                << " constructing from NUL-terminated UNICODE string: '"
                << zstr
                << "'\n"
                ;
#endif
    ...
    count_t String :: unicode ( const UTF8 * s, count_t elems )
    {
        ...
        UTF32 * utf32;
        Buffer buffer ( ( len + 1 ) * sizeof * utf32 );
    ...
    count_t String :: unicode ( const UTF16 * s, count_t elems )
    {
    ...
    count_t String :: unicode ( const UTF32 * s, count_t elems )
    {
        // BOM - not expected, but must be handled
        assert ( s != nullptr );
        bool bswap = false;
    ...
    String :: Wiper :: ~ Wiper ()
    {
        if ( wipe )
        {
            try
            {
                ...
                memset_s ( ( void * ) s . data (), bytes, 0, bytes );
            }
            catch ( ... )
            {
            }

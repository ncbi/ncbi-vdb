...
static std::string convertDigest(uint8_t const *digest, size_t size)
{
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        int n = snprintf(buf, 16, (i > 0 && i % 8 == 0) ? fmt : (fmt + 1), (int)digest[i]);
        result.append(buf, n);
    }
    return result;
}
...
TEST_CASE(SHA1_abc_NIST)
{
    ...
    std::string const expect{"A9993E36 4706816A BA3E2571 7850C26C 9CD0D89D"};
...
TEST_CASE(SHA224_abc_openssl)
{
    ...
    std::string const expect{"23097D22 3405D822 8642A477 BDA255B3 2AADBCE4 BDA0B3F7 E36C9DA7"};
...
TEST_CASE(SHA256_abc_NIST)
{
    ...
    std::string const expect{"BA7816BF 8F01CFEA 414140DE 5DAE2223 B00361A3 96177A9C B410FF61 F20015AD"};
    auto const got = convertDigest(digest, sizeof(digest));
    REQUIRE_EQ(got, expect);
}

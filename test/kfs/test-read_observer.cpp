...
public:
    ...
    )                                // to invalid invalid arguments
    {
        ...
        REQUIRE_RC(KDirectoryOpenFileRead(d, &file, "../vdb/db/VDB-3418.sra"));
        RELEASE(KDirectory, d);

        if (failures) {
            REQUIRE_RC_FAIL(KFileMakeMD5ReadObserver(file, NULL));
            REQUIRE_RC_FAIL(KFileMakeMD5ReadObserver(NULL, &_md5));
            REQUIRE_NULL(_md5);
        }
        ...
        bool failures = false )   // the same as in Start()
    {
        ...
        if (failures) {
            REQUIRE_RC_FAIL(KFileMD5ReadObserverGetDigest(NULL, digest,
                &error));
            REQUIRE_NULL(error);
            if (success)
                REQUIRE_RC(KFileMD5ReadObserverGetDigest(_md5, digest, NULL));
            else
                REQUIRE_RC_FAIL(
                    KFileMD5ReadObserverGetDigest(_md5, digest, NULL));
        }
...
TEST_CASE(ReadAllSkip0) {
    ...
    t.Finish(ERR_HEAD "0 of 12887839.");
}
...
//  REQUIRE_RC(KFileReadAll(t.file, 1, t.buf, 1, &num_read));
    ...
    t.Finish(ERR_HEAD "1 of 12887839.");
}
...
TEST_CASE(ReadAllTwice) {
    ...
    REQUIRE_RC(KFileReadAll(t.file, 1234567, t.buf, t.size, &num_read));
    t.Finish();
}
...
TEST_CASE(ReadAllTwicePartially) {
    ...
    REQUIRE_RC(KFileReadAll(t.file, 1000000, t.buf, t.size, &num_read));
    t.Finish();
}

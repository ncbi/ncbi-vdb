...
public:
    ...
    {
        THROW_ON_RC( KDBManagerOpenDBUpdate( m_mgr, & m_db, "%s", dbpath ) );
        THROW_ON_RC( KDatabaseOpenIndexUpdate ( m_db, &m_idx, name ) );
    }

    KDirectory * m_dir = nullptr;
    KDBManager * m_mgr = nullptr;
    KDatabase * m_db = nullptr;
    KIndex * m_idx = nullptr;
};
...
{   // noop on the write side
    ...
    REQUIRE_EQ( (int64_t)0, start_id );
    REQUIRE_EQ( (uint64_t)0, id_range );
    REQUIRE_EQ( (uint64_t)0, num_keys );

...
/**
* Unit tests for KStream and IPC-related functions
*/
...
FIXTURE_TEST_CASE ( KStreamAdaptersFileWriteOnly, KnsStreamFixture )
{
    ...
    REQUIRE_RC ( KStreamFromKFilePair ( & strm, nullptr, file ) );
...
FIXTURE_TEST_CASE ( KStreamAdaptersFileReadOnly, KnsStreamFixture )
{
    ...
    REQUIRE_RC ( KDirectoryOpenFileRead ( dir, ( const KFile ** ) & file, name ) );
    REQUIRE_RC ( KStreamFromKFilePair ( & strm, file, nullptr ) );
...
class EndpointFixture : public KNSManagerFixture
{
public:
    KEndPoint ep;
    String name;
...
FIXTURE_TEST_CASE(IPCEndpoint_Create, EndpointFixture)
{
    StringInitCString(&name, GetName());
    REQUIRE_RC(KNSManagerInitIPCEndpoint(m_mgr, &ep, &name));
    REQUIRE_EQ(ep.type, (KEndPointType)epIPC);
    REQUIRE_EQ(string(ep.u.ipc_name), string(GetName()));
}

FIXTURE_TEST_CASE(MakeListener, EndpointFixture)
{
    CONST_STRING(&name, "socket");
    REQUIRE_RC(KNSManagerInitIPCEndpoint(m_mgr, &ep, &name));
    KListener* listener;
    REQUIRE_RC(KNSManagerMakeListener( m_mgr, &listener, &ep ));
    REQUIRE_NOT_NULL(listener);
    KListenerRelease(listener);
}

FIXTURE_TEST_CASE(MakeIPCConnection_NoListener, EndpointFixture)
{
    CONST_STRING(&name, "socket");
    REQUIRE_RC(KNSManagerInitIPCEndpoint(m_mgr, &ep, &name));
    timeout_t tm;
    TimeoutInit ( & tm, 0 );
    KSocket* socket;
    REQUIRE_RC_FAIL(KNSManagerMakeRetryConnection(m_mgr, &socket, &tm, nullptr, &ep)); /* no server; no retries */
    REQUIRE_NULL(socket);
}
...
class SocketFixture : public EndpointFixture
{
...
public:
    ...
    : server(0), listener(0), threadWorker(0)
    {
        ...
        THROW_ON_RC (KNSManagerInitIPCEndpoint(m_mgr, &ep, &name) );
    ...
    rc_t Listen()
    {
        if (KNSManagerMakeListener ( m_mgr, &listener, &ep ) == 0)
        {
            rc_t rc = 0;
            while (rc == 0)
            {
    ...
    static rc_t DefaultWorkerThreadFn ( const KThread *self, void *data )
    {
        ...
        {   // this server worker converts the incoming message to all uppercase and sends it back
            ...
            size_t num;
            THROW_ON_RC (KStreamTimedRead(stream, localBuf, sizeof(localBuf), &num, nullptr) ); // wait forever
...
{   // client runs in a child process
    ...
    REQUIRE_RC(KStreamTimedRead(stream, buf, sizeof(buf), &num, nullptr));
...
{   // client runs in a child process
    ...
    REQUIRE_RC(KStreamTimedRead(stream2, buf, sizeof(buf), &num, nullptr));
...
public:
	...
	void SetupClient(const string& p_content)
	{
        ...
        StringInitCString(&name, SocketName.c_str());
        if (KNSManagerInitIPCEndpoint(m_mgr, &ep, &name) != 0)
    ...
    static rc_t StutteringWorkerThreadFn ( const KThread *self, void *data )
    {
        ...
        {   // this server worker converts the incoming message to all uppercase, pauses for SERVER_WRITE_DELAY_MS, and sends it back
            ...
            char localBuf[MaxMessageSize];
            size_t localNumRead;
            THROW_ON_RC ( KStreamTimedRead(stream, localBuf, sizeof(localBuf), &localNumRead, nullptr) ); // wait forever
            if (localNumRead == 0)
...
{   // 1.1. wait indefinitely until the server responds
    ...
    REQUIRE_RC(KStreamTimedWrite(m_stream, content.c_str(), content.length(), &num, nullptr)); // waits indefinitely
    LOG(LogLevel::e_message, "client after KStreamWrite" << endl);
    REQUIRE_EQ(content.length(), num);

    REQUIRE_RC(KStreamTimedRead(m_stream, buf, sizeof(buf), &num, nullptr)); // waits indefinitely
    LOG(LogLevel::e_message, "client after KStreamRead" << endl);
    REQUIRE_EQ(string(buf, num), ToUpper(content));
...
public:
    ...
    static rc_t TimedWriteServerFn ( const KThread *self, void *data )
    {
        ...
        char prefix[1024];
        string_printf ( prefix, sizeof prefix, nullptr, "TimedWriteSocketFixture worker %p: ", (void*)self );

...
#include "../../libs/kdbtext/KDBTextScanner.hpp"
...
#endif
TEST_SUITE ( KdbTextLexTestSuite );
#if 0
void
...
{
    REQUIRE_EQ ( (int)END_SOURCE, KDBTextScanner ( "" ) . NextToken () . GetType () );
}
#if 0
TEST_CASE ( Location )
{
...
{
    KDBTextScanner s ( "  bad", 2, false ); // only first 2 bytes looked at
    Token t = s . NextToken ();
    REQUIRE_EQ ( (int)END_SOURCE, t. GetType () );
    REQUIRE_EQ ( string ( "  " ), string ( t . GetLeadingWhitespace () ) );
}
...
{
    REQUIRE_EQ ( (int)UNRECOGNIZED, KDBTextScanner ( "ъ" ) . NextToken () . GetType () );
}

#define REQUIRE_LITERAL(name, lit) \
    TEST_CASE ( name ) { REQUIRE_EQ ( ( Token :: TokenType ) lit [ 0 ], KDBTextScanner ( lit ) . NextToken () . GetType () ); }
...
TEST_CASE ( Ellipsis )
{
    REQUIRE_EQ ( ( Token ::  TokenType ) ELLIPSIS, KDBTextScanner ( "..." ) . NextToken () . GetType () );
}
...
TEST_CASE ( name ) \
{ \
    Token t = KDBTextScanner ( term ) . NextToken (); \
...
TEST_CASE ( kw_##word ) \
{ \
    Token t = KDBTextScanner ( #word ) . NextToken (); \
...
TEST_CASE ( Comment )
{
    Token t = KDBTextScanner ( "/**/abc" ) . NextToken ();
...
const char UsageDefaultName[] = "wb-test-kdbtext-lex";
...
{
    return KdbTextLexTestSuite(argc, argv);
}

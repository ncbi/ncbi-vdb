...
void vdb_mbedtls_ecjpake_init( mbedtls_ecjpake_context *ctx )
{
    ...
    ctx->md_info = NULL;
    vdb_mbedtls_ecp_group_init( &ctx->grp );
    ctx->point_format = MBEDTLS_ECP_PF_UNCOMPRESSED;

    vdb_mbedtls_ecp_point_init( &ctx->Xm1 );
    vdb_mbedtls_ecp_point_init( &ctx->Xm2 );
    vdb_mbedtls_ecp_point_init( &ctx->Xp1 );
    vdb_mbedtls_ecp_point_init( &ctx->Xp2 );
    vdb_mbedtls_ecp_point_init( &ctx->Xp  );

    vdb_mbedtls_mpi_init( &ctx->xm1 );
    vdb_mbedtls_mpi_init( &ctx->xm2 );
    vdb_mbedtls_mpi_init( &ctx->s   );
}
...
void vdb_mbedtls_ecjpake_free( mbedtls_ecjpake_context *ctx )
{
    ...
    ctx->md_info = NULL;
    vdb_mbedtls_ecp_group_free( &ctx->grp );

    vdb_mbedtls_ecp_point_free( &ctx->Xm1 );
    vdb_mbedtls_ecp_point_free( &ctx->Xm2 );
    vdb_mbedtls_ecp_point_free( &ctx->Xp1 );
    vdb_mbedtls_ecp_point_free( &ctx->Xp2 );
    vdb_mbedtls_ecp_point_free( &ctx->Xp  );

    vdb_mbedtls_mpi_free( &ctx->xm1 );
    vdb_mbedtls_mpi_free( &ctx->xm2 );
    vdb_mbedtls_mpi_free( &ctx->s   );
}
                           ...
                           size_t len )
{
    ...
    if( ( ctx->md_info = vdb_mbedtls_md_info_from_type( hash ) ) == NULL )
        return( MBEDTLS_ERR_MD_FEATURE_UNAVAILABLE );

    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_group_load( &ctx->grp, curve ) );

    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_read_binary( &ctx->s, secret, len ) );

cleanup:
    if( ret != 0 )
        vdb_mbedtls_ecjpake_free( ctx );
                                    ...
                                    const mbedtls_ecp_point *P )
{
    ...
    ret = vdb_mbedtls_ecp_point_write_binary( grp, P, pf,
                                          &len, *p + 4, end - ( *p + 4 ) );
    if( ret != 0 )
        return( ret );
                         ...
                         mbedtls_mpi *h )
{
    ...
    *p++ = (unsigned char)( ( id_len       ) & 0xFF );
    ...
    /* Compute hash */
    MBEDTLS_MPI_CHK( vdb_mbedtls_md( md_info, buf, p - buf, hash ) );

    /* Turn it into an integer mod n */
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_read_binary( h, hash,
                                        vdb_mbedtls_md_get_size( md_info ) ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mod_mpi( h, h, &grp->N ) );
                             ...
                             const unsigned char *end )
{
    ...
    vdb_mbedtls_ecp_point_init( &V );
    vdb_mbedtls_ecp_point_init( &VV );
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_tls_read_point( grp, &V, p, end - *p ) );
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_read_binary( &r, *p, r_len ) );
    *p += r_len;
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_muladd( (mbedtls_ecp_group *) grp,
                     &VV, &h, X, &r, G ) );

    if( vdb_mbedtls_ecp_point_cmp( &VV, &V ) != 0 )
    {
...
cleanup:
    vdb_mbedtls_ecp_point_free( &V );
    vdb_mbedtls_ecp_point_free( &VV );
                              ...
                              void *p_rng )
{
    ...
    vdb_mbedtls_ecp_point_init( &V );
    vdb_mbedtls_mpi_init( &v );
    vdb_mbedtls_mpi_init( &h );

    /* Compute signature */
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_gen_keypair_base( (mbedtls_ecp_group *) grp,
                                                   G, &v, &V, f_rng, p_rng ) );
    MBEDTLS_MPI_CHK( ecjpake_hash( md_info, grp, pf, G, &V, X, id, &h ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( &h, &h, x ) ); /* x*h */
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_sub_mpi( &h, &v, &h ) ); /* v - x*h */
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mod_mpi( &h, &h, &grp->N ) ); /* r */

    /* Write it out */
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_tls_write_point( grp, &V,
                pf, &len, *p, end - *p ) );
    *p += len;

    len = vdb_mbedtls_mpi_size( &h ); /* actually r */
    ...
    *(*p)++ = (unsigned char)( len & 0xFF );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_write_binary( &h, *p, len ) ); /* r */
    *p += len;

cleanup:
    vdb_mbedtls_ecp_point_free( &V );
    vdb_mbedtls_mpi_free( &v );
    vdb_mbedtls_mpi_free( &h );
                             ...
                             const unsigned char *end )
{
     ...
     * } ECJPAKEKeyKP;
     */
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_tls_read_point( grp, X, p, end - *p ) );
    if( vdb_mbedtls_ecp_is_zero( X ) )
    {
                              ...
                              void *p_rng )
{
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_gen_keypair_base( (mbedtls_ecp_group *) grp, G, x, X,
                                                   f_rng, p_rng ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_tls_write_point( grp, X,
                pf, &len, *p, end - *p ) );
    *p += len;
                             ...
                             const mbedtls_ecp_point *C )
{
    ...
    vdb_mbedtls_mpi_init( &one );

    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_lset( &one, 1 ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_muladd( grp, R, &one, A, &one, B ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_muladd( grp, R, &one, R, &one, C ) );

cleanup:
    vdb_mbedtls_mpi_free( &one );
                                            ...
                                            size_t len )
{
    ...
    {
        MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_tls_read_group( &grp, &p, len ) );
                               ...
                               void *p_rng )
{
    ...
    vdb_mbedtls_mpi_init( &b );

    /* b = s + rnd-128-bit * N */
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_fill_random( &b, 16, f_rng, p_rng ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( &b, &b, N ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_add_mpi( &b, &b, S ) );

    /* R = sign * X * b mod N */
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( R, X, &b ) );
    R->s *= sign;
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mod_mpi( R, R, N ) );

cleanup:
    vdb_mbedtls_mpi_free( &b );
                            ...
                            void *p_rng )
{
    ...
    vdb_mbedtls_ecp_point_init( &G );
    vdb_mbedtls_ecp_point_init( &Xm );
    vdb_mbedtls_mpi_init( &xm );
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_mul( &ctx->grp, &Xm, &xm, &G, f_rng, p_rng ) );
    ...
    if( ctx->role == MBEDTLS_ECJPAKE_SERVER )
    {
        ...
        MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_tls_write_group( &ctx->grp, &ec_len,
                                                      p, end - p ) );
        p += ec_len;
    }
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_tls_write_point( &ctx->grp, &Xm,
...
cleanup:
    vdb_mbedtls_ecp_point_free( &G );
    vdb_mbedtls_ecp_point_free( &Xm );
    vdb_mbedtls_mpi_free( &xm );
                            ...
                            void *p_rng )
{
    ...
    *olen = vdb_mbedtls_md_get_size( ctx->md_info );
    if( len < *olen )
        return( MBEDTLS_ERR_ECP_BUFFER_TOO_SMALL );

    vdb_mbedtls_ecp_point_init( &K );
    vdb_mbedtls_mpi_init( &m_xm2_s );
    vdb_mbedtls_mpi_init( &one );
    ...
    MBEDTLS_MPI_CHK( ecjpake_mul_secret( &m_xm2_s, -1, &ctx->xm2, &ctx->s,
                                         &ctx->grp.N, f_rng, p_rng ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_muladd( &ctx->grp, &K,
                                         &one, &ctx->Xp,
                                         &m_xm2_s, &ctx->Xp2 ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_mul( &ctx->grp, &K, &ctx->xm2, &K,
                                      f_rng, p_rng ) );

    /* PMS = SHA-256( K.X ) */
    x_bytes = ( ctx->grp.pbits + 7 ) / 8;
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_write_binary( &K.X, kx, x_bytes ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_md( ctx->md_info, kx, x_bytes, buf ) );

cleanup:
    vdb_mbedtls_ecp_point_free( &K );
    vdb_mbedtls_mpi_free( &m_xm2_s );
    vdb_mbedtls_mpi_free( &one );
                              ...
                              const unsigned char *xm2, size_t len2 )
{
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_read_binary( &ctx->xm1, xm1, len1 ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_read_binary( &ctx->xm2, xm2, len2 ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_mul( &ctx->grp, &ctx->Xm1, &ctx->xm1,
                                      &ctx->grp.G, NULL, NULL ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_mul( &ctx->grp, &ctx->Xm2, &ctx->xm2,
                                      &ctx->grp.G, NULL, NULL ) );
...
int vdb_mbedtls_ecjpake_self_test( int verbose )
{
    ...
    TEST_ASSERT( vdb_mbedtls_ecjpake_setup( &srv, MBEDTLS_ECJPAKE_SERVER,
                    MBEDTLS_MD_SHA256, MBEDTLS_ECP_DP_SECP256R1,
                    ecjpake_test_password,
            sizeof( ecjpake_test_password ) ) == 0 );
    ...
    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_one( &srv, buf, len ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_write_round_one( &srv,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_one( &cli, buf, len ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_write_round_two( &srv,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_two( &cli, buf, len ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_derive_secret( &cli,
                 pms, sizeof( pms ), &pmslen, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_write_round_two( &cli,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_two( &srv, buf, len ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_derive_secret( &srv,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );
    ...
    /* Read round one */
    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_one( &srv,
                                    ecjpake_test_cli_one,
                            sizeof( ecjpake_test_cli_one ) ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_one( &cli,
                                    ecjpake_test_srv_one,
                            sizeof( ecjpake_test_srv_one ) ) == 0 );

    /* Skip generation of round two, read round two */
    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_two( &cli,
                                    ecjpake_test_srv_two,
                            sizeof( ecjpake_test_srv_two ) ) == 0 );

    TEST_ASSERT( vdb_mbedtls_ecjpake_read_round_two( &srv,

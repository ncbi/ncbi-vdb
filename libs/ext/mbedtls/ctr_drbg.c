 ...
 * vdb_mbedtls_ctr_drbg_seed_entropy_len(ctx, f_entropy, p_entropy,
                   ...
                   size_t entropy_len )
{
     ...
     */
    if( ( ret = vdb_mbedtls_aes_setkey_enc( &ctx->aes_ctx, key, MBEDTLS_CTR_DRBG_KEYBITS ) ) != 0 )
    {
        return( ret );
    }
                            ...
                            const unsigned char *data, size_t data_len )
{
    ...
    *p++ = MBEDTLS_CTR_DRBG_SEEDLEN;
    ...
    if( ( ret = vdb_mbedtls_aes_setkey_enc( &aes_ctx, key, MBEDTLS_CTR_DRBG_KEYBITS ) ) != 0 )
    {
        goto exit;
    }
    ...
    for( j = 0; j < MBEDTLS_CTR_DRBG_SEEDLEN; j += MBEDTLS_CTR_DRBG_BLOCKSIZE )
    {
        ...
        while( use_len > 0 )
        {
            ...
            if( ( ret = vdb_mbedtls_aes_crypt_ecb( &aes_ctx, MBEDTLS_AES_ENCRYPT, chain, chain ) ) != 0 )
            {
                goto exit;
            }
        }
     ...
     */
    if( ( ret = vdb_mbedtls_aes_setkey_enc( &aes_ctx, tmp, MBEDTLS_CTR_DRBG_KEYBITS ) ) != 0 )
    {
        goto exit;
    }
    ...
    for( j = 0; j < MBEDTLS_CTR_DRBG_SEEDLEN; j += MBEDTLS_CTR_DRBG_BLOCKSIZE )
    {
        if( ( ret = vdb_mbedtls_aes_crypt_ecb( &aes_ctx, MBEDTLS_AES_ENCRYPT, iv, iv ) ) != 0 )
        {
            goto exit;
        }
                              ...
                              const unsigned char data[MBEDTLS_CTR_DRBG_SEEDLEN] )
{
    ...
    for( j = 0; j < MBEDTLS_CTR_DRBG_SEEDLEN; j += MBEDTLS_CTR_DRBG_BLOCKSIZE )
    {
         ...
         * Crypt counter block
         */
        if( ( ret = vdb_mbedtls_aes_crypt_ecb( &ctx->aes_ctx, MBEDTLS_AES_ENCRYPT, ctx->counter, p ) ) != 0 )
            goto exit;
     ...
     */
    if( ( ret = vdb_mbedtls_aes_setkey_enc( &ctx->aes_ctx, tmp, MBEDTLS_CTR_DRBG_KEYBITS ) ) != 0 )
        goto exit;
 ...
 * vdb_mbedtls_ctr_drbg_reseed(ctx, additional, len)
 * implements
                              ...
                              const unsigned char *additional, size_t add_len )
{
    ...
    while( output_len > 0 )
    {
         ...
         * Crypt counter block
         */
        if( ( ret = vdb_mbedtls_aes_crypt_ecb( &ctx->aes_ctx, MBEDTLS_AES_ENCRYPT, ctx->counter, tmp ) ) != 0 )
            goto exit;

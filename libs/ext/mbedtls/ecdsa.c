...
#define ECDSA_BUDGET( ops )   \
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_check_budget( grp, ECDSA_RS_ECP, ops ) );
                ...
                mbedtls_ecdsa_restart_ctx *rs_ctx )
{
    ...
    if( ! vdb_mbedtls_ecdsa_can_do( grp->id ) || grp->N.p == NULL )
...
modn:
         ...
         */
        MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_gen_privkey( grp, &t, f_rng_blind,
                                                  p_rng_blind ) );
        ...
        MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mod_mpi( pk, pk, &grp->N ) );
...
sign:
    ...
    else
    {
        ...
        vdb_mbedtls_hmac_drbg_init( &rng_ctx_blind );
        p_rng_blind_det = &rng_ctx_blind;
        vdb_mbedtls_hmac_drbg_seed_buf( p_rng_blind_det, md_info,
                                    data, 2 * grp_len );
        ret = vdb_mbedtls_hmac_drbg_update_ret( p_rng_blind_det,
                                            (const unsigned char*) blind_label,
                                            strlen( blind_label ) );
        if( ret != 0 )
        {
            vdb_mbedtls_hmac_drbg_free( &rng_ctx_blind );
            goto cleanup;
        }
#else
        /*
         ...
         * through side channels. After vdb_mbedtls_ecdsa_sign_det() is removed NULL
         * won't be a valid value for f_rng_blind anymore. Therefore it should
         * be checked by the caller and this branch and check can be removed.
         */
        ret = ecdsa_sign_restartable( grp, r, s, d, buf, blen,
                                      vdb_mbedtls_hmac_drbg_random, p_rng,
                                      vdb_mbedtls_hmac_drbg_random, p_rng_blind_det,
                                      rs_ctx );
                  ...
                  int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    ret = vdb_mbedtls_ecp_group_load( &ctx->grp, gid );
    if( ret != 0 )
        return( ret );

   return( vdb_mbedtls_ecp_gen_keypair( &ctx->grp, &ctx->d,
                                    &ctx->Q, f_rng, p_rng ) );
}

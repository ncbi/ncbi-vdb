                ...
                int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    do
    {
        ...
        do
        {
            MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_gen_keypair( grp, &k, &R, f_rng, p_rng ) );
            MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mod_mpi( r, &R.X, &grp->N ) );
        ...
        {
            size_t n_size = ( grp->nbits + 7 ) / 8;
            MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_fill_random( &t, n_size, f_rng, p_rng ) );
            MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_shift_r( &t, 8 * n_size - grp->nbits ) );

            /* See vdb_mbedtls_ecp_gen_keypair() */
            if( ++blind_tries > 30 )
                return( MBEDTLS_ERR_ECP_RANDOM_FAILED );
        }
        while( vdb_mbedtls_mpi_cmp_int( &t, 1 ) < 0 ||
               vdb_mbedtls_mpi_cmp_mpi( &t, &grp->N ) >= 0 );

        /*
         * Step 6: compute s = (e + r * d) / k = t (e + rd) / (kt) mod n
         */
        MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( s, r, d ) );
        MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_add_mpi( &e, &e, s ) );
        MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( &e, &e, &t ) );
        MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( &k, &k, &t ) );
        MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_inv_mod( s, &k, &grp->N ) );
                    ...
                    mbedtls_md_type_t md_alg )
{
    ...
    vdb_mbedtls_hmac_drbg_seed_buf( &rng_ctx, md_info, data, 2 * grp_len );

    ret = vdb_mbedtls_ecdsa_sign( grp, r, s, d, buf, blen,
                      vdb_mbedtls_hmac_drbg_random, &rng_ctx );
                  ...
                  const mbedtls_ecp_point *Q, const mbedtls_mpi *r, const mbedtls_mpi *s)
{
    ...
    vdb_mbedtls_ecp_point_init( &R );
    vdb_mbedtls_mpi_init( &e ); vdb_mbedtls_mpi_init( &s_inv ); vdb_mbedtls_mpi_init( &u1 ); vdb_mbedtls_mpi_init( &u2 );
     ...
     */
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_check_pubkey( grp, Q ) );
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( &u1, &e, &s_inv ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mod_mpi( &u1, &u1, &grp->N ) );

    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mul_mpi( &u2, r, &s_inv ) );
    MBEDTLS_MPI_CHK( vdb_mbedtls_mpi_mod_mpi( &u2, &u2, &grp->N ) );

    /*
     * Step 5: R = u1 G + u2 Q
     *
     * Since we're not using any secret data, no need to pass a RNG to
     * vdb_mbedtls_ecp_mul() for countermesures.
     */
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecp_muladd( grp, &R, &u1, &grp->G, &u2, Q ) );
...
cleanup:
    vdb_mbedtls_ecp_point_free( &R );
    vdb_mbedtls_mpi_free( &e ); vdb_mbedtls_mpi_free( &s_inv ); vdb_mbedtls_mpi_free( &u1 ); vdb_mbedtls_mpi_free( &u2 );
 ...
 */
int vdb_mbedtls_ecdsa_write_signature( mbedtls_ecdsa_context *ctx, mbedtls_md_type_t md_alg,
                           ...
                           void *p_rng )
{
    ...
    MBEDTLS_MPI_CHK( vdb_mbedtls_ecdsa_sign_det( &ctx->grp, &r, &s, &ctx->d,
                             hash, hlen, md_alg ) );
#else
    (void) md_alg;

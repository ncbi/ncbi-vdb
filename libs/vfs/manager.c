                             ...
                             bool is_refseq )
{
    ...
    if ( rc == 0 )
    {
        ...
        caching_params cps;
        get_caching_params( &cps, blocksize, is_refseq );
    ...
    if ( rc == 0 )
    {
        ...
        if ( self->resolver != NULL )
        {
            ...
            if ( rc == 0 )
            {
                ...
                rc = VFSManagerMakeHTTPFile( self, f, uri->addr, local_cache->path.addr, high_reliability, blocksize, is_refseq );
                {
                ...
                rc = VFSManagerMakeHTTPFile( self, f, uri->addr, NULL, high_reliability, blocksize, is_refseq );
        }
        else
        {
...
LIB_EXPORT rc_t CC VFSManagerOpenFileRead ( const VFSManager *self,
                                            ...
                                            const VPath * path_ )
{
    return VFSManagerOpenFileReadWithBlocksize ( self, f, path_, DEFAULT_CACHE_PAGE_SIZE );
}
                                      ...
                                      bool force_decrypt)
{
    ...
    rc = VFSManagerOpenCurlFile ( self, &file, path, DEFAULT_CACHE_PAGE_SIZE );
    if ( rc != 0 )
    {

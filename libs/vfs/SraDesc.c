...
static rc_t SraDescInit(SraDesc * self) {
    rc_t rc = 0;
...
static rc_t SraDescWhack(SraDesc * self) {
    rc_t rc = 0;
...
static rc_t SraDescLoadAsText(SraDesc * self, uint64_t size) {
    ...
    for (n.u = 0, from = 0; rc == 0 && from < size && buf[from] != '\n';
        ++from)
    {
        if (buf[from] < '0' || buf[from] > '9')
            return RC(rcExe, rcFile, rcReading, rcData, rcInvalid);
        n.u = n.u * 10 + buf[from] - '0';
    }
    self->_quality = q;
    self->_size = n.u;
    return 0;
...
static const char * SDExt(const PrfOutFile * self) {
    switch (self->_tfType) {
    case eTextual:
        return EXT_TXT;
    case eBin:
...
static rc_t SraDescOpen(SraDesc * self, const char * sra) {
    assert(self && sra);
    STSMSG(STS_DBG, ("opening %s%s", sra, SDExt(self)));
    rc = KDirectoryOpenFileRead(self->_dir, &self->_file, true, "%s%s",
        sra, SDExt(self));
    if (rc == 0) {
        uint64_t fsize = 0;
...
#ifdef DEBUGGING
        OUTMSG(("%s: %s%s found: loading...\n", __FUNCTION__, sra, SDExt(self)));
#endif
        STSMSG(STS_DBG, ("loading %s%s", sra, SDExt(self)));

        rc = KFileSize(self->_file, &fsize);
        if (rc != 0)
            PLOGERR(klogInt, (klogInt, rc, "Cannot Size($(arg)$(ext))",
                "arg=%s,ext=%s", sra, SDExt(self)));
        else {
            rc = KDataBufferResize(&self->_buf, fsize);
            if (rc == 0) {
                rc = KFileReadExactly(self->_file, 0, self->_buf.base, fsize);
                if (rc != 0) {
                    PLOGERR(klogInt, (klogInt, rc, "Cannot Read($(arg)$(ext))",
                        "arg=%s,ext=%s", sra, SDExt(self)));
                    return rc;
                }
            }
...
static rc_t SraDescLoad(SraDesc * self) {
    switch (self->_tfType) {
    case eTextual:
        SraDescLoadAsText(self);
        break;
    case eBinEol:
        SraDescLoadAsBinl(self);
        break;
    default:
        assert(0);
        break;
...
rc_t ConvertBinToTxt(const uint8_t * in, uint64_t sz, KFile * out)
{
    ...
    rc = string_printf(buf, sizeof buf, &num_writ, "%c", in[from] + '0');
    if (rc == 0) {
...
static rc_t ConvertTxtToBin(const char * in, uint64_t sz, KFile * out) {
    ...
    if (in[from] != '1' && in[from] != '3' && in[from] != '4')
        return RC(rcExe, rcFile, rcReading, rcData, rcInvalid);
    q = in[from] - '0';
    ++from;
    if (in[from++] != '\n')
    ...
    if (rc == 0) {
        rc = KFileWrite(out, to, &q, sizeof q, &num_writ);
        to += num_writ;
    }
    if (rc == 0)

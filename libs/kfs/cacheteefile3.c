...
static rc_t CC KSQueObjDispose_please_do_not_call_that_method ( KSQueObj *self )
{
    if ( self != NULL ) {
 ...
 */
static rc_t CC KSQueObjMake_please_do_not_call_that_method ( KSQueObj **Obj,
    size_t InitialPageIndex, size_t LastPageIndex, bool ReadyToRead )
{
    rc_t rc;
    KSQueObj *Ret;
...
static rc_t CC KSQueObjAddRef ( KSQueObj *self )
{
    if ( self != NULL ) { atomic32_inc ( &( self->refcount ) ); }
    return 0;
} /* KSQueObjAddRef () */

static rc_t CC KSQueObjRelease ( KSQueObj *self )
{
    if ( self != NULL ) {
        if ( atomic32_dec_and_test ( &( self->refcount ) ) ) {
            return KSQueObjDispose_please_do_not_call_that_method ( self );
        }
    }
    return 0;
} /* KSQueObjRelease () */

static rc_t CC KSQueObjWait ( KSQueObj *self, struct timeout_t *Tm )
{
    rc_t rc = 0;
    ...
    if ( self->ready_to_read ) {
        ...
        if ( rc == 0 ) {
            STATUS (
                STAT_PRG, "%s - sque obj waiting on condition\n", __func__ );
            rc = KConditionTimedWait ( self->obj_cond, self->obj_lock, Tm );
            if ( rc != 0 ) {
                STATUS (
                    STAT_QA, "%s - timed wait failed: %R\n", __func__, rc );
            }
...
static rc_t CC KSQueObjNotify ( KSQueObj *self )
{
    if ( self == NULL ) {
...
static rc_t CC KSQueClear ( KSQue *self )
{
    rc_t rc;
    KSQueObj *Obj;
    ...
    if ( rc == 0 ) {
        while (
            ( Obj = (KSQueObj *)DLListPopHead ( &( self->sque ) ) ) != NULL ) {
 ...
 */
static bool CC KSQueSealed ( KSQue *self )
{
    return self == NULL ? false : ( atomic32_read ( &( self->sealed ) ) != 0 );
} /* KSQueSealed () */
#endif /* JOJOBA */

static rc_t CC KSQueSeal ( KSQue *self )
{
    if ( self == NULL ) {
...
static rc_t CC KSQueDispose ( KSQue *self )
{
    if ( self != NULL ) {
 ...
 */
static rc_t CC KSQueMake ( KSQue **CeQue, size_t PagesQty )
{
    rc_t rc;
    KSQue *Ret;
    ...
    if ( rc != 0 ) {
        *CeQue = NULL;
        ...
        if ( Ret != NULL ) { KSQueDispose ( Ret ); }
    }

    return rc;
} /* KSQueMake () */

static rc_t CC KSQueNewObj (
    KSQue *self, KSQueObj **Obj, size_t InitialPageIndex, size_t LastPageIndex )
{
    rc_t rc;
    KSQueObj *Ret;
    bool ReadyToRead;
    ...
    rc = KSQueObjMake_please_do_not_call_that_method (
        &Ret, InitialPageIndex, LastPageIndex, ReadyToRead );
    if ( rc == 0 ) {
        if ( ReadyToRead ) {
            *Obj = Ret;
        } else {
            ...
            if ( rc == 0 ) {
                ...
                rc = KLockAcquire ( self->sque_lock );
                if ( rc == 0 ) {
                    DLListPushTail ( &( self->sque ), &( Ret->dad ) );
                    ...
                    *Obj = Ret;
                    ...
                    if ( rc != 0 ) {
                        PLOGERR ( klogSys,
                            ( klogSys, rc,
                                "BG: $(func) - failed to signal resume reading",
                                "func=%s", __func__ ) );
    ...
    if ( rc != 0 ) {
        *Obj = NULL;
        ...
        if ( Ret != NULL ) { KSQueObjRelease ( Ret ); }
    }

    return rc;
} /* KSQueNewObj () */

static rc_t CC KSQueNotify ( KSQue *self, size_t LastPageIndex )
{
    rc_t rc;
    DLList new_list;
    KSQueObj *Obj;
    ...
    if ( rc == 0 ) {
        if ( LastPageIndex <= self->last_available_page_index ) {
            ...
            while ( ( Obj = (KSQueObj *)DLListPopHead ( &( self->sque ) ) )
                != NULL ) {
                 ...
                 */
                if ( Obj->last_page_idx <= self->last_available_page_index ) {
...
static rc_t CC KSQueWaitForRead ( KSQue *self )
{
    rc_t rc = 0;
...
static bool KCacheTeeFilePageInCache (
    const KCacheTeeFile_v3 *self, const size_t pg_idx );

static bool is_bitmap_full ( KCacheTeeFile_v3 *self )
{
    const size_t num_pages = self->source_size / self->page_size;

    for ( size_t pg_idx = 0; pg_idx != num_pages; ++pg_idx )
        if ( !KCacheTeeFilePageInCache ( self, pg_idx ) ) return false;
    return true;
}
...
{
    rc_t rc = 0;
    fprintf ( stderr, "promotion logic goes here\n" );
    if ( !is_bitmap_full ( self ) ) return 0;
    fprintf ( stderr, " OK to promote %s\n", self->path );
    /*
    rc = KDirectoryOpenFileSharedWrite ( dir, & self -> cache_file, true,
    "%s.cache", self -> path ); if (rc) return rc;
    */

    /* self->path, KFile self->cache_file renamed and truncated to source_size*/
    char cache_file_name[4096];
    size_t num_writ = 0;
    rc = string_printf ( cache_file_name, sizeof cache_file_name, &num_writ,
        "%s.cache", self->path );
    if ( rc ) return rc;

    char temp_file_name[4096];
    rc = string_printf ( temp_file_name, sizeof temp_file_name, &num_writ,
        "%s.cache.temp", self->path );
    if ( rc ) return rc;

    /* Cache file released in dtor */
    ...
    */
    ...
    rc = KFileSetSize ( temp_file, self->source_size );
    if ( rc ) return rc;

    /* (5) releaes open temp. cache file ( windows cannot rename open files ) */
    rc = KFileRelease ( temp_file );
    if ( rc ) return rc;

    /* (6) rename to final filename ( windows cannot rename open files ) */
    rc = KDirectoryRename ( self->dir, true, temp_file_name, self->path );
    if ( rc ) return rc;

    return 0;
}

static int64_t CC KCacheTeeFileTreeNodeFind (
    const void *item, const BSTNode *n )
{
...
static int64_t CC KCacheTeeFileTreeNodeSort (
    const BSTNode *item, const BSTNode *n )
{
...
#define ENTER_CRIT_SECTION() pthread_mutex_lock ( &crit )
#define EXIT_CRIT_SECTION() pthread_mutex_unlock ( &crit )
#endif
static BSTree open_cache_tee_files;

static rc_t CC KCacheTeeChunkReaderDestroy ( KCacheTeeChunkReader *self )
{
...
static size_t CC KCacheTeeChunkReaderBufferSize (
    const KCacheTeeChunkReader *self )
{
    return self->ctf->page_size;
}

static rc_t CC KCacheTeeChunkReaderNext (
    KCacheTeeChunkReader *self, void **buf, size_t *size )
{
...
static rc_t KCacheTeeFileRAMCacheInsert (
    KCacheTeeFile_v3 *self, const void *buf, size_t pg_idx )
{
    rc_t rc;
    ...
    if ( self->ram_limit == 0 ) {
        ...
        return SILENT_RC (
            rcFS, rcNode, rcInserting, rcFunction, rcNotAvailable );
    }
    ...
    if ( rc == 0 ) {
        ...
        if ( existing != NULL ) {
            ...
            STATUS (
                STAT_PRG, "BG: %s - retriving existing MRU node\n", __func__ );
            rc = KVectorGetPtr (
                self->ram_cache_mru_idx, pg_idx, (void **)&lmru );
            if ( rc == 0 ) {
                /* 5. reuse to mark as MRU */
                STATUS ( STAT_PRG,
                    "BG: %s - relinking MRU node to head of list\n", __func__ );
        ...
        /* if cache is already full ... */
            ...
            lmru = (KCacheTeeFileLMRUPage *)DLListPopTail (
                &self->ram_cache_mru );
            ...
            STATUS ( STAT_PRG,
                "BG: %s - clearing cache and mru-index vector entries\n",
                __func__ );
            ...
            if ( self->cache_file == NULL ) {
                STATUS ( STAT_PRG,
                    "BG: %s - clear page %zu present in bitmap\n", __func__,
                    pg_idx );
                self->bitmap[pg_idx >> BMWORDBITS]
                    &= ~( 1U << ( pg_idx & BMWORDMASK ) );
            }

            /* 10. reuse node and insert new guy as MRU */
            STATUS ( STAT_PRG,
                "BG: %s - reusing MRU node and placing at head of list\n",
                __func__ );
            lmru->pg_idx = pg_idx;
            DLListPushHead ( &self->ram_cache_mru, &lmru->dad );
        ...
        /* cache is not full */
            ...
            if ( lmru == NULL )
                ...
                lmru->pg_idx = pg_idx;
                STATUS ( STAT_PRG,
                    "BG: %s - inserting MRU node into index @ %zu\n", __func__,
                    pg_idx );
                rc = KVectorSetPtr ( self->ram_cache_mru_idx, pg_idx, lmru );
                if ( rc == 0 ) {
                    /* 13. insert as MRU */
                    STATUS ( STAT_PRG,
                        "BG: %s - placing MRU node into head of list\n",
                        __func__ );
                    DLListPushHead ( &self->ram_cache_mru, &lmru->dad );
                    ...
                    ++self->ram_pg_count;
                    STATUS ( STAT_PRG,
                        "BG: %s - new RAM cache page count is %u\n", __func__,
                        self->ram_pg_count );
                }
            }
        }
...
static rc_t KCacheTeeFileCacheInsert (
    KCacheTeeFile_v3 *self, uint64_t pos, const void *buf, size_t size )
{
...
static rc_t KCacheTeeFileSaveBitmap ( KCacheTeeFile_v3 *self )
{
    rc_t rc = 0;
    ...
    if ( self->cache_file != NULL ) {
        ...
        STATUS (
            STAT_GEEK, "BG: %s - saved bm result code %R\n", __func__, rc );
    }

    return rc;
}

static rc_t CC KCacheTeeChunkReaderConsume (
    KCacheTeeChunkReader *chunk, uint64_t pos, const void *buf, size_t size )
{
    rc_t rc;
    ...
    if ( rc == 0 ) {
        ...
        STATUS (
            STAT_PRG, "BG: %s - insert buffer into RAM cache\n", __func__ );
        ...
        if ( rc2 != 0 ) {
            STATUS ( STAT_PRG,
                "BG: %s - buffer not inserted into RAM cache - freeing\n",
                __func__ );
            free ( (void *)buf );
        }
        ...
        if ( rc2 == 0 || rc3 == 0 ) {
            ...
            STATUS ( STAT_PRG,
                "BG: %s - broadcasting event to all waiting readers\n",
                __func__ );
...
static rc_t CC KCacheTeeChunkReaderReturn (
    KCacheTeeChunkReader *self, void *buf, size_t size )
{
...
static KChunkReader_vt_v1 KCacheTeeChunkReader_vt = {
    ...
    KCacheTeeChunkReaderDestroy, KCacheTeeChunkReaderBufferSize,
    KCacheTeeChunkReaderNext, KCacheTeeChunkReaderConsume,
    KCacheTeeChunkReaderReturn
    /* end minor version == 0 */
};

static rc_t KCacheTeeFileMakeChunkReader ( KCacheTeeFile_v3 *self )
{
    rc_t rc;
    ...
    if ( obj == NULL )
        ...
        rc = KChunkReaderInit (
            &obj->dad, (const KChunkReader_vt *)&KCacheTeeChunkReader_vt );
        if ( rc == 0 ) {
...
static rc_t CC KCacheTeeFileDestroy ( KCacheTeeFile_v3 *self )
{
    if ( self->cache_file != NULL ) {
        int status = ENTER_CRIT_SECTION ();
        if ( status == 0 ) {
            BSTNode *node = BSTreeFind (
                &open_cache_tee_files, self->path, KCacheTeeFileTreeNodeFind );
            if ( node != NULL ) {
    ...
    /* ? promote cache_file ? */
    /* TBD */
    if ( self->promote ) promote_cache ( self );
...
static struct KSysFile_v1 *CC KCacheTeeFileGetSysFile (
    const KCacheTeeFile_v3 *self, uint64_t *offset )
{
    return NULL;
}

static rc_t CC KCacheTeeFileRandomAccess ( const KCacheTeeFile_v3 *self )
{
    if ( self->cache_file != NULL )
...
static rc_t CC KCacheTeeFileGetSize (
    const KCacheTeeFile_v3 *self, uint64_t *size )
{
    *size = self->source_size;
    return 0;
}

static rc_t CC KCacheTeeFileSetSize ( KCacheTeeFile_v3 *self, uint64_t size )
{
    ...
    const KCacheTeeFile_v3 *self, const size_t pg_idx )
{
    ...
    assert (
        ( pg_idx / BMWORDSIZE ) * sizeof self->bitmap[0] < self->bmap_size );
    return ( self->bitmap[pg_idx >> BMWORDBITS]
               & ( 1 << ( pg_idx & BMWORDMASK ) ) )
        != 0;
}

static size_t KCacheTeeFileReadFromRAM ( const KCacheTeeFile_v3 *self,
    uint64_t pos, uint8_t *buffer, size_t bsize, size_t *num_read,
    size_t initial_page_idx )
{
    ...
    STATUS (
        STAT_PRG, "%s - reading contiguous pages from RAM cache\n", __func__ );
    for ( i = total = 0; total < bsize; total += num_copied, ++i ) {
        ...
        STATUS ( STAT_PRG, "%s - retrieving page %zu from RAM cache\n",
            __func__, initial_page_idx + i );
        rc = KVectorGetPtr (
            self->ram_cache, initial_page_idx + i, (void **)&pg );
        if ( rc != 0 ) {
...
static uint32_t bmword_contig_bits (
    const bmap_t word, const uint32_t initial_bit_pos, bool *found_zero )
{
    ...
    */
    ...
    if ( first_zero_pos < 0 ) return BMWORDSIZE - first_one_pos;
...
static uint32_t KCacheTeeFileContigPagesInFileCache (
    const KCacheTeeFile_v3 *self, size_t initial_page_idx, size_t end_page_idx )
{
    size_t i, last;
    bool found_zero;
    ...
    STATUS ( STAT_GEEK,
        "%s - initial page idx=%zu, end page idx=%zu, initial word idx=%zu\n",
        __func__, initial_page_idx, end_page_idx, i );

    /* generate mask against left-most word */
    initial_bit_pos = ( uint32_t ) ( initial_page_idx & BMWORDMASK );
    /* TBD - this will break in 64-bit */
    initial_mask
        = ( uint32_t ) ( 0xFFFFFFFFU << ( initial_page_idx & BMWORDMASK ) );
    STATUS ( STAT_GEEK, "%s - initial word bitpos=%u, initial mask=0b%0*lb\n",
        __func__, initial_bit_pos, BMWORDSIZE, (uint64_t)initial_mask );
    ...
    STATUS ( STAT_GEEK,
        "%s - initial contiguous page count=%u, found zero=%s\n", __func__,
        count, found_zero ? "true" : "false" );
    ...
    for ( ++i; i <= last; ++i ) {
        ...
        STATUS ( STAT_GEEK, "%s - contiguous page count=%u, found zero=%s\n",
            __func__, count, found_zero ? "true" : "false" );
...
static rc_t KCacheTeeFileReadFromFile ( const KCacheTeeFile_v3 *self,
    ...
    size_t initial_page_idx )
{
    ...
    end_page_idx = ( size_t ) (
        ( pos + bsize + self->page_size - 1 ) / self->page_size );

    /* we would not be called if there were no pages available to read */
    STATUS ( STAT_PRG,
        "%s - counting number of contiguous pages in cache file\n", __func__ );
    num_contig_pages = KCacheTeeFileContigPagesInFileCache (
        self, initial_page_idx, end_page_idx );
    STATUS ( STAT_PRG, "%s - count is %u starting from idx %zu\n", __func__,
        num_contig_pages, initial_page_idx );
    ...
    if ( pos + bsize > end ) bsize = ( size_t ) ( end - pos );
...
static rc_t CC KCacheTeeFileTimedRead ( const KCacheTeeFile_v3 *self,
    uint64_t pos, void *buffer, size_t bsize, size_t *num_read,
    struct timeout_t *tm )
{
    rc_t rc = 0;
    ...
    STATUS ( STAT_GEEK,
        "%s - allocating sque object "
        "{ pos=%lu, size=%zu, tm=%d%s, initial_page_idx=%zu, last_page_idx=%zu "
        "}\n",
        __func__, pos, bsize, ( tm != NULL ) ? (int)tm->mS : -1,
        ( tm != NULL ) ? "mS (present)" : " (infinite)", initial_page_idx,
        last_page_idx );

    rc = KSQueNewObj ( self->sque, &Obj, initial_page_idx, last_page_idx );
    if ( rc != 0 ) {
        /* Apparently if there is error code returned,
         * the Obj will be NULL ... so just exiting
         */
        STATUS (
            STAT_QA, "%s - sque object allocation failed: %R\n", __func__, rc );
        return rc;
    }
    ...
    count = KCacheTeeFileReadFromRAM (
        self, pos, buffer, bsize, num_read, initial_page_idx );
    if ( count == 0 ) {
...
static rc_t CC KCacheTeeFileRead ( const KCacheTeeFile_v3 *self, uint64_t pos,
...
static rc_t CC KCacheTeeFileWrite ( KCacheTeeFile_v3 *self, uint64_t pos,
...
static uint32_t CC KCacheTeeFileGetType ( const KCacheTeeFile_v3 *self )
{
    if ( self->cache_file != NULL ) return KFileType ( self->cache_file );
    return KFileType ( self->source );
}

static rc_t CC KCacheTeeFileTimedWrite ( KCacheTeeFile_v3 *self, uint64_t pos,
...
static rc_t CC KCacheTeeFileReadChunked ( const KCacheTeeFile_v3 *self,
    uint64_t pos, KChunkReader *chunks, size_t bsize, size_t *total_read )
{
    rc_t rc = 0;
    ...
    for ( total = 0; rc == 0 && total < bsize; total += num_read ) {
        ...
        if ( rc == 0 ) {
            ...
            if ( total + chsize > bsize ) to_read = bsize - total;

            STATUS ( STAT_PRG, "%s - reading from file @ %lu\n", __func__,
                pos + total );
            rc = KFileReadAll_v1 (
                &self->dad, pos + total, chbuf, to_read, &num_read );
            if ( rc == 0 && num_read != 0 ) {
                STATUS ( STAT_PRG, "%s - consuming chunk of %zu bytes @ %lu\n",
                    __func__, num_read, pos + total );
                rc = KChunkReaderConsumeChunk (
                    chunks, pos + total, chbuf, num_read );
            }
...
static rc_t CC KCacheTeeFileTimedReadChunked ( const KCacheTeeFile_v3 *self,
    uint64_t pos, KChunkReader *chunks, size_t bsize, size_t *total_read,
    struct timeout_t *tm )
{
    rc_t rc = 0;
    ...
    STATUS (
        STAT_PRG, "%s - timed chunked read from cache-tee file\n", __func__ );
    ...
    for ( total = 0; rc == 0 && total < bsize; total += num_read ) {
        ...
        if ( rc == 0 ) {
            ...
            STATUS ( STAT_PRG, "%s - reading from file @ %lu\n", __func__,
                pos + total );
            rc = KFileTimedReadAll_v1 (
                &self->dad, pos + total, chbuf, to_read, &num_read, tm );
            if ( rc == 0 && num_read != 0 ) {
                STATUS ( STAT_PRG, "%s - consuming chunk of %zu bytes @ %lu\n",
                    __func__, num_read, pos + total );
                rc = KChunkReaderConsumeChunk (
...
static KFile_vt_v1 KCacheTeeFile_v3_vt = {
    ...
    /* start minor version == 0 */
    KCacheTeeFileDestroy, KCacheTeeFileGetSysFile, KCacheTeeFileRandomAccess,
    KCacheTeeFileGetSize, KCacheTeeFileSetSize, KCacheTeeFileRead,
    KCacheTeeFileWrite,
    /* end minor version == 0 */
    ...
    KCacheTeeFileTimedRead, KCacheTeeFileTimedWrite,
    /* end minor version == 2 */

    /* start minor version == 3 */
    KCacheTeeFileReadChunked, KCacheTeeFileTimedReadChunked
    /* end minor version == 3 */
};

/* RunThread
 */
static rc_t KCacheTeeFileBGLoop ( KCacheTeeFile_v3 *self )
{
    rc_t rc = 0;
    ...
    while ( !self->quitting ) {
        ...
        rc = KSQueLastRequestedPageIndex ( self->sque, &LastRequestedPage );
        if ( rc != 0 ) {
            STATUS ( STAT_PRG,
                "BG: %s - can not find last requested page inded.\n",
                __func__ );
        } else {
            if ( KCacheTeeFilePageInCache ( self, LastRequestedPage ) ) {
                STATUS (
                    STAT_PRG, "BG: %s - waiting for next read\n", __func__ );
                rc = KSQueWaitForRead ( self->sque );
            } else {
                size_t num_read = 0;
                uint64_t InitialPos = initial_page_index * self->page_size;
                size_t SizeRead = ( LastRequestedPage - initial_page_index )
                    * self->page_size;
                if ( SizeRead < min_read_amount ) {
                    SizeRead = min_read_amount;
                }
                rc = KFileTimedReadChunked ( self->source, InitialPos,
                    self->chunks, SizeRead, &num_read, NULL /* size */
                );
                STATUS ( STAT_PRG, "BG: %s - rc=%R, num_read=%zu\n", __func__,
                    rc, num_read );
                if ( rc == 0 ) {
                    initial_page_index
                        += ( size_t ) ( num_read + self->page_size - 1 )
                        / self->page_size;
                }
            }
        }
    ...
    STATUS (
        STAT_PRG, "BG: %s - exiting loop and background thread\n", __func__ );

    return rc;
}

static rc_t CC KCacheTeeFileRunThread ( const KThread *t, void *data )
{
    rc_t rc;
    ...
    if ( rc != 0 ) {
        PLOGERR ( klogSys,
            ( klogSys, rc, "BG: $(func) - failed to acquire mutex", "func=%s",
                __func__ ) );
        self->quitting = true;
        return rc;
    }
    ...
    if ( rc != 0 ) {
        PLOGERR ( klogSys,
            ( klogSys, rc, "BG: $(func) - failed to signal foreground",
                "func=%s", __func__ ) );
        self->quitting = true;
        return rc;
    }
 ...
 */
static rc_t KCacheTeeFileStartBgThread ( KCacheTeeFile_v3 *self )
{
    rc_t rc;
    ...
    if ( rc != 0 ) {
        PLOGERR ( klogSys,
            ( klogSys, rc, "$(func) - failed to acquire mutex", "func=%s",
                __func__ ) );
    } else {
        ...
        if ( rc != 0 ) {
            PLOGERR ( klogSys,
                ( klogSys, rc, "$(func) - failed to start background thread",
                    "func=%s", __func__ ) );
        } else {
            ...
            if ( rc != 0 ) {
                PLOGERR ( klogSys,
                    ( klogSys, rc,
                        "$(func) - failed to wait on condition signal",
                        "func=%s", __func__ ) );
            }
        }
...
static rc_t KCacheTeeFileInitSync ( KCacheTeeFile_v3 *self )
{
    rc_t rc;

    size_t end_page_idx = ( size_t ) (
        ( self->source_size + self->page_size - 1 ) / self->page_size );
    ...
    if ( rc != 0 ) {
        PLOGERR ( klogSys,
            ( klogSys, rc, "$(func) - failed to allocate sque", "func=%s",
                __func__ ) );
    } else {
        ...
        if ( rc != 0 ) {
            PLOGERR ( klogSys,
                ( klogSys, rc, "$(func) - failed to create mutex", "func=%s",
                    __func__ ) );
        } else {
            ...
            if ( rc != 0 ) {
                PLOGERR ( klogSys,
                    ( klogSys, rc, "$(func) - failed to create condition",
                        "func=%s", __func__ ) );
            }
        }
    }
...
static rc_t KCacheTeeFileInitNew ( KCacheTeeFile_v3 *self )
{
    rc_t rc;
    ...
    if ( rc != 0 ) {
        PLOGERR ( klogSys,
            ( klogSys, rc,
                "$(func) - failed to set file size of '$(path).cache' to "
                "$(eof) bytes",
                "func=%s,path=%s,eof=%lu", __func__, self->path,
                calculated_eof ) );
    } else {
        uint64_t pos;
        ...
        STATUS ( STAT_PRG,
            "%s - writing initial cache file tail at offset %lu\n", __func__,
            pos );
        rc = KFileWriteExactly (
            self->cache_file, pos, self->tail, sizeof *self->tail );
        if ( rc != 0 ) {
            PLOGERR ( klogSys,
                ( klogSys, rc,
                    "$(func) - failed to write file tail of '$(path).cache'",
                    "func=%s,path=%s", __func__, self->path ) );
        }
    }
...
static rc_t KCacheTeeFileInitExisting ( KCacheTeeFile_v3 *self )
{
    rc_t rc;
    ...
    STATUS ( STAT_GEEK, "%s - file size = %lu, rc = $R\n", __func__, actual_eof,
        rc );
    if ( rc != 0 ) {
        PLOGERR ( klogSys,
            ( klogSys, rc,
                "$(func) - failed to obtain file size of '$(path).cache'",
                "func=%s,path=%s", __func__, self->path ) );
    } else {
        uint64_t calculated_eof;
        ...
        calculated_eof
            = self->source_size + self->bmap_size + sizeof *self->tail;
        STATUS ( STAT_GEEK, "%s - calculated size = %lu\n", __func__,
            calculated_eof );
        if ( calculated_eof != actual_eof ) {
            STATUS ( STAT_QA,
                "%s - actual size ( %lu ) != required size ( %lu )\n", __func__,
                actual_eof, calculated_eof );
            ...
            STATUS ( STAT_PRG,
                "%s - reading tail of shared cache file '%s.cache'\n", __func__,
                self->path );
            rc = KFileReadExactly (
                self->cache_file, pos, &existing_tail, sizeof existing_tail );
            STATUS ( STAT_GEEK,
                "%s - read results: { orig_size = %lu, page_size = %u }, rc = "
                "%R\n",
                __func__, existing_tail.orig_size, existing_tail.page_size,
                rc );
            if ( rc != 0 ) {
                PLOGERR ( klogSys,
                    ( klogSys, rc,
                        "$(func) - failed to read tail of '$(path).cache'",
                        "func=%s,path=%s", __func__, self->path ) );
            } else if ( self->source_size != existing_tail.orig_size
                || self->page_size != existing_tail.page_size ) {
                PLOGMSG ( klogWarn,
                    ( klogWarn,
                        "$(func) - cache file parameters have changed for "
                        "'$(path).cache'",
                        "func=%s,path=%s", __func__, self->path ) );
                ...
                STATUS ( STAT_PRG,
                    "%s - reading bitmap of shared cache file '%s.cache'\n",
                    __func__, self->path );
...
static rc_t KCacheTeeFileInitShared ( KCacheTeeFile_v3 *self )
{
    rc_t rc;
    ...
    PLOGMSG ( klogWarn,
        ( klogWarn,
            "$(func) - stale cache file '$(path).cache'. Reinitializing.",
            "func=%s,path=%s", __func__, self->path ) );
    ...
    if ( rc != 0 ) {
        ...
        if ( rc != 0 ) {
            PLOGERR ( klogSys,
                ( klogSys, rc,
                    "$(func) - failed to reinitialize '$(path).cache'",
                    "func=%s,path=%s", __func__, self->path ) );
        }
    }
...
static rc_t KCacheTeeFileOpen (
    KCacheTeeFile_v3 *self, KDirectory *dir, const KFile **promoted )
{
    rc_t rc;
    uint64_t dummy;
    (void)dummy;
    ...
    STATUS ( STAT_PRG,
        "%s - duplicating directory reference %#p for possible promotion\n",
        __func__, dir );
    ...
    if ( rc == 0 ) {
        ...
        for ( wait_loop_count = 0; wait_loop_count < 10; ++wait_loop_count ) {
            ...
            if ( rc == 0 )
                STATUS (
                    STAT_QA, "%s - file '%s' exists\n", __func__, self->path );
            else {
                STATUS ( STAT_PRG,
                    "%s - attempting to lock file '%s' for update\n", __func__,
                    self->path );
                rc = KDirectoryCreateLockFile (
                    dir, &lock, "%s.lock", self->path );
                ...
                if ( rc != 0 ) {
                    STATUS ( STAT_QA,
                        "%s - failed to acquire lock for '%s' - %R\n", __func__,
                        self->path, rc );
                    return rc;
                }

                STATUS ( STAT_PRG,
                    "%s - attempting to open file '%s.cache' shared "
                    "read/write\n",
                    __func__, self->path );
                rc = KDirectoryOpenFileSharedWrite (
                    dir, &self->cache_file, true, "%s.cache", self->path );
                STATUS ( STAT_GEEK,
                ...
                if ( rc == 0 )
                    ...
                    STATUS ( STAT_PRG,
                        "%s - attempting to create file '%s.cache' "
                        "read/write\n",
                        __func__, self->path );
                    /* TBD - if this fails, go back to open-shared-write */
                    rc = KDirectoryCreateFile ( dir, &self->cache_file, true,
                        0666, kcmCreate | kcmParents, "%s.cache", self->path );
                    STATUS ( STAT_GEEK,
                        "%s - create file attempt: fd = %d, rc = %R\n",
                        __func__,
                        KFileGetSysFile ( self->cache_file, &dummy )->fd, rc );
                    if ( rc == 0 ) rc = KCacheTeeFileInitNew ( self );
                }
                ...
                if ( self->cache_file == NULL && rc != 0 ) {
                    ...
                    rc = KDirectoryOpenFileRead ( dir,
                        (const KFile **)&self->cache_file, "%s.cache",
                        self->path );
                    STATUS ( STAT_GEEK,
                        "%s - open read-only file attempt: fd = %d, rc = $R\n",
                        __func__,
                        KFileGetSysFile ( self->cache_file, &dummy )->fd, rc );
                    if ( rc == 0 ) rc = KCacheTeeFileInitExisting ( self );
                }
                ...
                if ( rc != 0 ) {
                    PLOGERR ( klogSys,
                        ( klogSys, rc,
                            "$(func) - failed to open cache file "
                            "'$(path).cache'",
                            "func=%s,path=%s", __func__, self->path ) );
...
static rc_t KCacheTeeFileMakeBitmap ( KCacheTeeFile_v3 *self )
{
    size_t bmsize;
    size_t num_words;
    uint64_t num_pages;
    ...
    if ( bitmap == NULL ) {
        ...
        PLOGERR ( klogSys,
            ( klogSys, rc, "$(func) - failed to allocate $(size) bytes",
                "func=%s,size=%zu", __func__, bmsize ) );
        return rc;
    }
...
static rc_t KCacheTeeFileMakeRAMCache ( KCacheTeeFile_v3 *self )
{
    rc_t rc;
...
static rc_t KCacheTeeFileBindSourceFile (
    KCacheTeeFile_v3 *self, const KFile *source )
{
    rc_t rc;

    STATUS ( STAT_PRG, "%s - obtaining source file size\n", __func__ );
    rc = KFileSize ( source, &self->source_size );
    STATUS ( STAT_PRG, "%s - source file size returned %lu bytes, rc = %R\n",
        __func__, self->source_size, rc );
    if ( rc != 0 ) {
        PLOGERR ( klogSys,
            ( klogSys, rc, "$(func) - failed to obtain source file size",
                "func=%s", __func__ ) );
    } else {
        ...
        if ( rc == 0 ) {
            self->source = source;
            STATUS ( STAT_GEEK, "%s - file reference %#p has refcount = %u\n",
                __func__, source, atomic32_read ( &source->refcount ) );
...
static void KCacheTeeFileBindConstants ( KCacheTeeFile_v3 *self,
    size_t page_size, uint32_t cluster_factor, uint32_t ram_pages, bool promote,
    bool temporary )
{
    ...
    if ( cluster_factor == 0 ) cluster_factor = DEFAULT_CLUSTER_FACT;
...
static rc_t KDirectoryVMakeKCacheTeeFileInt ( KDirectory *self,
    const KFile **tee, const KFile *source, size_t page_size,
    uint32_t cluster_factor, uint32_t ram_pages, bool promote, bool temporary,
    const char *nul_term_cache_path )
{
    ...
    STATUS (
        STAT_PRG, "%s - allocating %u byte object\n", __func__, sizeof *obj );
    obj = malloc ( sizeof *obj );
    if ( obj == NULL ) {
        rc = RC ( rcFS, rcFile, rcAllocating, rcMemory, rcExhausted );
        PLOGERR ( klogSys,
            ( klogSys, rc,
                "$(func) - failed to allocate $(bytes) bytes for object",
                "func=%s,bytes=%zu", __func__, sizeof *obj ) );
    } else {
        ...
        if ( rc != 0 ) {
            free ( obj );
            PLOGERR ( klogInt,
                ( klogInt, rc, "$(func) - failed to bind vtable to object",
                    "func=%s", __func__ ) );
        } else {
            /* bind the parameters and constants to object */
            KCacheTeeFileBindConstants (
                obj, page_size, cluster_factor, ram_pages, promote, temporary );
            ...
            if ( rc == 0 ) {
                ...
                if ( rc == 0 ) {
                    ...
                    if ( rc == 0 ) {
                        ...
                        if ( nul_term_cache_path == NULL
                            || nul_term_cache_path[0] == 0 )
                            STATUS ( STAT_PRG,
                                "%s - no file cache will be used\n", __func__ );
                        else {
                            size_t cache_path_size
                                = string_copy_measure ( obj->path,
                                    sizeof obj->path, nul_term_cache_path );
        ...
        if ( self == NULL )
            ...
            if ( fmt == NULL || fmt[0] == 0 ) {
                ...
                if ( ram_pages == 0 ) {
                    ...
                    STATUS ( STAT_QA,
                        "%s - no RAM cache or file cache will be used\n",
                        __func__ );
                    rc = KFileAddRef ( source );
                    if ( rc == 0 ) *tee = source;
                    return rc;
                ...
                rc = KDirectoryVMakeKCacheTeeFileInt ( self, tee, source,
                    page_size, cluster_factor, ram_pages, promote, temporary,
                    fmt );
            } else {
                KCacheTeeFileTreeNode *new_node = malloc ( sizeof *new_node );
                if ( new_node == NULL )
                    rc = RC (
                        rcFS, rcFile, rcConstructing, rcMemory, rcExhausted );
                else {
                    /* expand cache path */
                    ...
                    if ( rc != 0 ) {
                        PLOGERR ( klogSys,
                            ( klogSys, rc,
                                "$(func) - failed to resolve cache file path",
                                "func=%s", __func__ ) );
                    } else {
                        /* protect cache FILE from multiple threads and
                         * processes */
                        int status = ENTER_CRIT_SECTION ();
                        if ( status != 0 )
                            rc = RC ( rcFS, rcDirectory, rcConstructing, rcLock,
                                rcUnknown );
                        else {
                            const KCacheTeeFileTreeNode *node
                                = (const KCacheTeeFileTreeNode *)BSTreeFind (
                                    &open_cache_tee_files, new_node->path,
                                    KCacheTeeFileTreeNodeFind );
                            if ( node != NULL ) {
                                free ( new_node );
                                rc = KFileAddRef ( node->file );
                                if ( rc == 0 ) *tee = node->file;
                            } else {
                                rc = KDirectoryVMakeKCacheTeeFileInt ( self,
                                    &new_node->file, source, page_size,
                                    cluster_factor, ram_pages, promote,
                                    temporary, new_node->path );
                                if ( rc != 0 )
                                    free ( new_node );
                                else {
                                    rc = BSTreeInsertUnique (
                                        &open_cache_tee_files, &new_node->dad,
                                        NULL, KCacheTeeFileTreeNodeSort );
                                    if ( rc == 0 )
                                        *tee = new_node->file;
                                    else {
    ...
    const char *fmt, ... )
{
    ...
    rc = KDirectoryVMakeKCacheTeeFile_v3 ( self, tee, source, page_size,
        cluster_factor, ram_pages, promote, temporary, fmt, args );

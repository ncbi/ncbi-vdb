...
static rc_t CC KSQueClear ( KSQue *self )
{
    ...
    if ( rc == 0 )
    {
        while ( ( Obj = (KSQueObj *)DLListPopHead ( &( self->sque ) ) ) != NULL )
        {
...
static rc_t CC KSQueSeal ( KSQue *self )
{
    ...
    STATUS ( STAT_PRG, "%s - notifying reader\n", __func__ );
...
static rc_t CC KSQueNewObj ( KSQue *self, KSQueObj **Obj, size_t InitialPageIndex,
                             size_t LastPageIndex )
{
    ...
    rc = KSQueObjMake_please_do_not_call_that_method ( &Ret, InitialPageIndex, LastPageIndex, ReadyToRead );
    if ( rc == 0 )
    {
        if ( ReadyToRead )
        {
        ...
        else
        {
            ...
            if ( rc == 0 )
            {
                ...
                if ( rc == 0 )
                {
                    ...
                    if ( LastPageIndex < self->pages_qty && self->last_requested_page_index < LastPageIndex )
                    {
...
static bool KCacheTeeFilePageInCache ( const KCacheTeeFile_v3 *self, const size_t pg_idx );
...
static bool is_bitmap_full ( KCacheTeeFile_v3 *self )
{
    ...
    {
        if ( !KCacheTeeFilePageInCache ( self, pg_idx ) )
            return false;
    }
    return true;
}
...
static rc_t promote_cache ( KCacheTeeFile_v3 *self )
{
    ...
    */
    ...
    rc = KDirectoryOpenFileWrite ( self->dir, &temp_file, true, "%s", temp_file_name );
    if ( rc ) return rc;
...
static int64_t CC KCacheTeeFileTreeNodeFind ( const void *item, const BSTNode *n )
{
...
static int64_t CC KCacheTeeFileTreeNodeSort ( const BSTNode *item, const BSTNode *n )
{
...
static size_t CC KCacheTeeChunkReaderBufferSize ( const KCacheTeeChunkReader *self )
{
    return self->ctf->page_size;
}

static rc_t CC KCacheTeeChunkReaderNext ( KCacheTeeChunkReader *self, void **buf, size_t *size )
{
    ...
    const KCacheTeeFile_v3 *self, size_t initial_page_idx, size_t end_page_idx )
{
    ...
    for ( ++i; i <= last; ++i )
    {
        ...
        word = self->bitmap[i];
        STATUS ( STAT_GEEK, "%s - word[%zu]=0b%0*lb\n", __func__, i, BMWORDSIZE, (uint64_t)word );
    ...
    while ( !self->quitting )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                size_t SizeRead = ( LastRequestedPage - initial_page_index ) * self->page_size;
...
static rc_t KCacheTeeFileInitExisting ( KCacheTeeFile_v3 *self )
{
    ...
    else
    {
        ...
        else
        {
            ...
            STATUS ( STAT_GEEK, "%s - read results: { orig_size = %lu, page_size = %u }, rc = "
                "%R\n",
    ...
    if ( rc == 0 )
    {
        ...
        for ( wait_loop_count = 0; wait_loop_count < 10; ++wait_loop_count )
        {
            ...
            else
            {
                ...
                STATUS ( STAT_GEEK, "%s - open shared file attempt: fd = %d, rc = %R\n",
                         __func__,
                        ( rc == 0 )
                ...
                else if ( GetRCState ( rc ) == rcNotFound )
                {
                    ...
                    STATUS ( STAT_GEEK, "%s - create file attempt: fd = %d, rc = %R\n",
                             __func__,
                ...
                {
                    STATUS ( STAT_PRG, "%s - attempting to open file '%s.cache' read-only\n",
                             __func__, self->path );
                    rc = KDirectoryOpenFileRead ( dir, (const KFile **)&self->cache_file, "%s.cache", self->path );
                    STATUS ( STAT_GEEK,
            ...
            if ( rc == 0 )
            {
                ...
                if ( rc == 0 )
                {
                    ...
                    if ( rc == 0 )
                    {
                        ...
                        {
                            size_t cache_path_size = string_copy_measure ( obj->path,
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            const KCacheTeeFileTreeNode *node = (const KCacheTeeFileTreeNode *)BSTreeFind (
                            ...
                            else
                            {
                                ...
                                {
                                    rc = BSTreeInsertUnique ( &open_cache_tee_files, &new_node->dad,

...
rc_t CC KCacheTeeChunkReaderNext ( KCacheTeeChunkReader * self, void ** buf, size_t * size )
{
    if ( self -> ctf -> quitting )
    {
        ...
        * buf = NULL;
        * size = 0;
printf("KCacheTeeChunkReaderNext(%p) quitting\n", (void*)self);
...
printf("KCacheTeeFileDestroy(%p) set quitting\n", (void*)self);
    self -> quitting = true;
...
printf("KCacheTeeFileRunThread(%p)set quitting 1\n", (void*)self);
        self -> quitting = true;
        return rc;
    }
...
printf("KCacheTeeFileRunThread(%p) set quitting 2\n", (void*)self);
        self -> quitting = true;
        return rc;
    }
        ...
        else
        {
            ...
            if ( fmt == NULL || fmt [ 0 ] == 0 )
            {
                ...
                if ( ram_pages == 0 )
                {
                    ...
                    if ( rc == 0 )
                        * tee = source;
printf("KDirectoryVMakeKCacheTeeFile_v3(%p) no caching\n", (void*)*tee);
                    return rc;
                }
...
printf("KDirectoryVMakeKCacheTeeFile_v3(%p)\n", (void*)*tee);
            }
            else
            {
...
printf("KDirectoryVMakeKCacheTeeFile_v3(%p) physical %s\n", (void*)*tee, new_node -> path);

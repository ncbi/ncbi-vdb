 ...
 * KConfig
 */
static rc_t KConfigSever ( const KConfig *self );
...
static void CC KConfigIncludedWhack ( BSTNode *n, void *ignore ) { free ( n ); }

static int64_t CC KConfigIncludedSort ( const BSTNode *item, const BSTNode *n )
{
...
/* Whack
 */
static void CC KConfigNodeWhack ( BSTNode *n, void *data )
{
...
/* Init
 */
static void KConfigNodeInit ( KConfigNode *self, const String *name )
{
    ...
    string_copy (
        (char *)( self + 1 ), name->size + 1, name->addr, name->size );
    StringInit ( &self->name, (char *)( self + 1 ), name->size, name->len );
    StringInit ( &self->value, "", 0, 0 );
    KRefcountInit (
        &self->refcount, 0, "KConfigNode", "init", self->name.addr );
}

/* Make
 */
static rc_t KConfigNodeMake ( KConfigNode **n, const String *name )
{
    ...
    if ( cn == NULL ) {
        ...
        PLOGERR ( klogErr,
            ( klogErr, rc, "Unable to create a config item for $(i)", "i=%S",
                name ) );
        return rc;
    }
 ...
 * Sort
 */
static int64_t CC KConfigNodeCmp ( const void *item, const BSTNode *n )
{
...
static int64_t CC KConfigNodeSort ( const BSTNode *item, const BSTNode *n )
{
...
rc_t KConfigAppendToLoadPath ( KConfig *self, const char *chunk )
{
    ...
    if ( chunk == NULL || chunk[0] == '\0' ) { return rc; }
    ...
    if ( self->load_path[0] != '\0' ) { strcat ( self->load_path, ":" ); }
...
/* Whack
 */
static rc_t KConfigEmpty ( KConfig *self )
{
    if ( self ) {
...
static rc_t find_home_directory (
    KDyld *dyld, const KDirectory **dir, bool release_cached_dir )
{
...
static rc_t KConfigWhack ( KConfig *self )
{
    bool release_cached_dir = true;
    find_home_directory ( NULL, NULL, release_cached_dir );

    if ( self == G_kfg.ptr ) atomic_test_and_set_ptr ( &G_kfg, NULL, self );
...
/* Init
 */
static void KConfigInit ( KConfig *self, KConfigNode *root )
{
...
LIB_EXPORT rc_t CC KConfigRelease ( const KConfig *self )
{
    if ( self != NULL ) {
        ...
        case krefWhack: return KConfigWhack ( (KConfig *)self );
        case krefLimit:
...
static KConfig *KConfigAttach ( const KConfig *self )
{
    if ( self != NULL ) {
        switch ( KDualRefAddDep ( &self->refcount, "KConfig" ) ) {
        case krefLimit: return NULL;
        }
    }
    return (KConfig *)self;
}
...
/* init_token_source
 */
static rc_t init_token_source ( KTokenText *tt, KTokenSource *src, char *full,
    size_t fsize, const char *srcpath, const char *path, va_list args )
{
    size_t num_writ;
    rc_t rc = 0;
...
/* Find
 */
static KToken *KConfigNodeFind ( const KConfigNode *self, const KConfigNode **n,
    KTokenSource *src, KToken *t )
{
    *n = NULL;
...
/* Create
 */
static KToken *KConfigNodeCreate ( KConfigNode *self, KConfigNode **n,
    KTokenSource *src, KToken *t, KConfigIncluded *current_file )
{
    bool created = false;
    KConfigNode *nself;
    *n = NULL;
    ...
    if ( created ) self->came_from = current_file;
 ...
 */
static rc_t KConfigNodeVOpenNodeReadInt ( const KConfigNode *self,
    const KConfig *mgr, const KConfigNode **node, const char *path,
    va_list args )
{
    rc_t rc;
    ...
    if ( node == NULL ) {
        ...
        PLOGERR ( klogErr,
            ( klogErr, rc, "faile to provide node to open $(n)", "n=%s",
                path ) );
    } else {
        if ( self == NULL ) {
            ...
            PLOGERR ( klogErr,
                ( klogErr, rc,
                    "failed to provide node reference for opening $(n)", "n=%s",
                    path ) );
        } else {
            if ( path == NULL || path[0] == 0 ) {
                *node = self;
                ...
                rc = init_token_source (
                    &tt, &src, full, sizeof full, "", path, args );
                if ( rc == 0 ) {
                    /* look ahead */
                    KToken t;
                    ...
                    if ( KConfigNodeFind ( self, node, &src, &t ) == NULL ) {
                        ...
                        PLOGERR ( klogErr,
                            ( klogErr, rc, "bad path $(p)", "p=%s", path ) );
                    }
                    if ( ( self = *node ) == NULL ) {
                        rc = SILENT_RC (
                            rcKFG, rcNode, rcOpening, rcPath, rcNotFound );
                        /* don't complain about this
                           PLOGERR (klogErr, (klogErr, rc, "can't find symbol
                           $(p)", "p=%s", path));
                        */
...
{
    if ( self == NULL ) return RC ( rcKFG, rcNode, rcOpening, rcSelf, rcNull );
    if ( mgr == NULL ) return RC ( rcKFG, rcNode, rcOpening, rcParam, rcNull );
    *mgr = self->mgr;
    ...
    const KConfigNode **node, const char *path, va_list args )
{
    if ( self != NULL )
        return KConfigNodeVOpenNodeReadInt (
            self, self->mgr, node, path, args );

    if ( node == NULL ) return RC ( rcKFG, rcNode, rcOpening, rcParam, rcNull );

    *node = NULL;
    return RC ( rcKFG, rcNode, rcOpening, rcSelf, rcNull );
}

LIB_EXPORT rc_t CC KConfigNodeOpenNodeRead (
    const KConfigNode *self, const KConfigNode **node, const char *path, ... )
{
    rc_t rc;
    va_list args;
...
LIB_EXPORT rc_t CC KConfigOpenNodeRead (
    const KConfig *self, const KConfigNode **node, const char *path, ... )
{
    rc_t rc;
    va_list args;
 ...
 *  special values NULL and "" are interpreted as "."
 */
static rc_t KConfigNodeVOpenNodeUpdateInt ( KConfigNode *self, KConfig *mgr,
            ...
            if ( path == NULL || path[0] == 0 ) {
                *node = self;
                ...
                if ( rc == 0 ) {
                    ...
                    /* follow path */
                    assert ( mgr != NULL );
                    if ( KConfigNodeCreate (
                             self, node, &src, &t, mgr->current_file )
                        == NULL )
                        return RC (
                            rcKFG, rcNode, rcOpening, rcPath, rcInvalid );
                    if ( ( self = *node ) == NULL )
                        rc = RC (
                            rcKFG, rcNode, rcOpening, rcMemory, rcExhausted );
                    else if ( t.id != eEndOfInput )
                        rc = RC ( rcKFG, rcNode, rcOpening, rcPath, rcInvalid );
                }
            }
...
LIB_EXPORT rc_t CC KConfigNodeVOpenNodeUpdate (
    KConfigNode *self, KConfigNode **node, const char *path, va_list args )
{
    if ( self != NULL )
...
LIB_EXPORT rc_t CC KConfigNodeOpenNodeUpdate (
    KConfigNode *self, KConfigNode **node, const char *path, ... )
{
    rc_t rc;
    va_list args;
...
LIB_EXPORT rc_t CC KConfigVOpenNodeUpdate (
    KConfig *self, KConfigNode **node, const char *path, va_list args )
{
    rc_t rc;
...
LIB_EXPORT rc_t CC KConfigOpenNodeUpdate (
    KConfig *self, KConfigNode **node, const char *path, ... )
{
    rc_t rc;
    va_list args;
 ...
 */
LIB_EXPORT rc_t CC KConfigNodeRead ( const KConfigNode *self, size_t offset,
    char *buffer, size_t bsize, size_t *num_read, size_t *remaining )
{
    rc_t rc;
    size_t dummy;

    if ( remaining == NULL ) remaining = &dummy;
...
static void KConfigNodeSetDirty ( KConfigNode *self )
{
 ...
 */
LIB_EXPORT rc_t CC KConfigNodeWrite (
    KConfigNode *self, const char *buffer, size_t size )
{
    rc_t rc;
...
KFG_EXTERN rc_t CC KConfigNodeWriteBool ( KConfigNode *self, bool state )
{
    ...
    if ( rc == 0 ) rc = KConfigNodeWrite ( self, value, written );
    return rc;
}
 ...
 */
LIB_EXPORT rc_t CC KConfigNodeAppend (
    KConfigNode *self, const char *buffer, size_t size )
{
    rc_t rc;
    ...
    if ( self == NULL )
        ...
        void *new_buffer
            = realloc ( self->val_buffer, self->value.size + size + 1 );
        if ( new_buffer == NULL )
            return RC ( rcKFG, rcNode, rcWriting, rcMemory, rcExhausted );
        self->val_buffer = new_buffer;
        string_copy ( &self->val_buffer[self->value.size],
            self->value.size + size + 1, buffer, size );
        self->value.size += size;
 ...
 */
LIB_EXPORT rc_t CC KConfigNodeReadAttr ( const KConfigNode *self,
    const char *name, char *buffer, size_t bsize, size_t *size )
{
    PLOGMSG (
        klogFatal, ( klogFatal, "$(F) unimplemented", "F=%s", __func__ ) );
    return -1;
}
 ...
 */
LIB_EXPORT rc_t CC KConfigNodeWriteAttr (
    KConfigNode *self, const char *name, const char *value )
{
    PLOGMSG (
        klogFatal, ( klogFatal, "$(F) unimplemented", "F=%s", __func__ ) );
    return -1;
...
{
    if ( self == NULL ) return RC ( rcKFG, rcNode, rcClearing, rcSelf, rcNull );
...
LIB_EXPORT rc_t CC KConfigNodeVDropChild (
    KConfigNode *self, const char *path, va_list args )
{
    PLOGMSG (
        klogFatal, ( klogFatal, "$(F) unimplemented", "F=%s", __func__ ) );
    return -1;
...
LIB_EXPORT rc_t CC KConfigNodeDropChild (
    KConfigNode *self, const char *path, ... )
{
    PLOGMSG (
        klogFatal, ( klogFatal, "$(F) unimplemented", "F=%s", __func__ ) );
    return -1;
 ...
 */
LIB_EXPORT rc_t CC KConfigNodeRenameAttr (
    KConfigNode *self, const char *from, const char *to )
{
    PLOGMSG (
        klogFatal, ( klogFatal, "$(F) unimplemented", "F=%s", __func__ ) );
    return -1;
...
LIB_EXPORT rc_t CC KConfigNodeRenameChild (
    KConfigNode *self, const char *from, const char *to )
{
    PLOGMSG (
        klogFatal, ( klogFatal, "$(F) unimplemented", "F=%s", __func__ ) );
...
static rc_t update_node (
    KConfig *self, const char *key, const char *value, TInternal internal )
{
    KConfigNode *node;
    rc_t rc = KConfigOpenNodeUpdate ( self, &node, "%s", key );
    if ( rc == 0 ) {
        /*        pLogMsg(klogInfo, "updating config key $(KEY) with
           '$(VALUE)'", "KEY=%s,VALUE=%s", key, value);*/
...
static rc_t write_nvp (
    void *pself, const char *name, size_t nameLen, VNamelist *values )
{ /* concatenate all values from the namelist and put the result into config
    ...
    for ( i = 0; i < count; ++i ) {
        ...
        string_copy (
            buf + concatTo, size + 1 - concatTo, val, string_size ( val ) );
    ...
    { /* create the node */
        ...
        /* some old config files may have "dbGaP" in their repository keys
         * misspelled as "dbGap" - fix if seen */
        const char *oldGaPprefix = "/repository/user/protected/dbGap-";
        size_t size = sizeof ( "/repository/user/protected/dbGap-" ) - 1;
        bool needsFix = string_cmp ( name, string_size ( name ), oldGaPprefix,
                            size, (uint32_t)size )
            == 0;
        ...
        if ( needsFix ) ( (char *)( nameStr->addr ) )[size - 2] = 'P';

        rc = update_node ( self, nameStr->addr, buf, false );
        if ( needsFix ) {
            KConfigNode *node;
            rc = KConfigOpenNodeUpdate ( self, &node, "%s", nameStr->addr );
            if ( rc == 0 ) { /* we are likely to be initializing, so have to set
                                the dirty flags directly, not through
                                KConfigNodeSetDirty() */
                self->dirty = true;
                node->dirty = true;
                KConfigNodeRelease ( node );
...
static bool look_up_var ( void *self, struct KFGToken *pb )
{
    const KConfigNode *node;
    rc_t rc = KConfigOpenNodeRead ( (KConfig *)self, &node, "%.*s",
        pb->tokenLength - 3, pb->tokenText + 2 );
    if ( rc == 0 ) {
...
static void CC report_error ( KFGScanBlock *sb, const char *msg )
{
    pLogMsg ( klogErr,
        "$(file):$(line):$(column): error: token='$(token)', msg='$(msg)'",
        "file=%s,line=%d,column=%d,token=%.*s,msg=%s", sb->file,
        sb->lastToken->line_no, sb->lastToken->column_no,
        sb->lastToken->tokenLength, sb->lastToken->tokenText, msg );
}

#define DISP_RC2( rc, name, msg )                                              \
    (void)( ( rc == 0 ) ? 0                                                    \
                        : PLOGERR ( klogInt,                                   \
                              ( klogInt, rc, "$(name): $(msg)",                \
                                  "name=%s,msg=%s", name, msg ) ) )
 ...
 */
static void PrintBuffInit ( PrintBuff *pb, KFile *f )
{
    assert ( pb != NULL );
...
static rc_t PrintBuffFlush ( PrintBuff *self )
{
    ...
    if ( self->buffered != 0 ) {
        ...
        if ( self->f == NULL ) {
            ...
            rc = KFileWriteAll ( self->f, self->flushed, self->buffer,
        ...
        if ( rc == 0 ) {
            if ( num_writ != self->buffered )
                rc = RC (
                    rcKFG, rcBuffer, rcFlushing, rcTransfer, rcIncomplete );
            else {
 ...
 */
static rc_t PrintBuffPrint ( PrintBuff *self, const char *fmt, ... )
{
    rc_t rc;
    size_t num_writ;
    va_list args1, args2;
    ...
    if ( rc == 0 )
        ...
        if ( rc == 0 ) {
            ...
            if ( rc == 0 ) self->buffered += num_writ;
        }
    }
...
static rc_t KConfigNodeReadData (
    const KConfigNode *self, char *buf, size_t blen, size_t *num_read )
{
    rc_t rc = 0;
    size_t remaining = 0;
...
static rc_t CC PrintBuffPrintQuoted ( PrintBuff *self, const String *data )
{
    ...
    for ( i = 0; i < StringLength ( data ); ++i ) {
        ...
        if ( str[i] < ' ' ) {
            rc = PrintBuffPrint (
                self, "\\x%c%c", ToHex ( str[i] / 16 ), ToHex ( str[i] % 16 ) );
        } else {
            switch ( str[i] ) {
            case '"': rc = PrintBuffPrint ( self, "\\\"" ); break;
            default: rc = PrintBuffPrint ( self, "%c", str[i] );
            }
        }
    }
    if ( rc == 0 ) rc = PrintBuffPrint ( self, "\"" );
    return rc;
}
    ...
    bool native, const char *fullpath, bool hide, PrintBuff *pb )
{
    bool secret = false;
    {
        ...
        if ( ( string_cmp (
                   name, string_measure ( name, NULL ), d1, l1, (uint32_t)l1 )
                 == 0 )
            || ( string_cmp (
                     name, string_measure ( name, NULL ), d2, l2, (uint32_t)l2 )
                   == 0 )
            || ( string_cmp (
                     name, string_measure ( name, NULL ), d3, l3, (uint32_t)l3 )
                   == 0 ) ) {
            secret = true;
        }
    }
    if ( hide && !native && secret ) {
        const char *ellipsis = "";
        const char replace[] = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        ...
        if ( rc == 0 ) { rc = PrintBuffPrint ( pb, "\n" ); }
        return rc;
    }
}


#define RELEASE( type, obj )                                                   \
    do {                                                                       \
        rc_t rc2 = type##Release ( obj );                                      \
        if ( rc2 && !rc ) { rc = rc2; }                                        \
        obj = NULL;                                                            \
    } while ( false )
    ...
    const KConfig *withIncluded )
{
    ...
    if ( !native ) {
        ...
        if ( rc == 0 ) {
            ...
            if ( skipCount > 0 ) va_end ( args_copy );
            if ( found ) return rc;
            ...
            if ( withIncluded ) {
                ...
                if ( rc == 0 ) rc = KNamelistCount ( names, &count );
                if ( rc == 0 ) {
                    uint32_t i = 0;
                    rc = printIndent ( indent, pb );
                    PrintBuffPrint ( pb, "\n  <ConfigurationFiles>\n" );
                    for ( i = 0; i < count && rc == 0; ++i ) {
                        ...
                        if ( rc == 0 ) rc = KNamelistGet ( names, i, &name );
                        if ( rc == 0 ) {
                            PrintBuffPrint ( pb, "%s\n", name );
                            //                          hasAny = true;
                        }
    ...
    if ( rc == 0 ) {
        ...
        if ( rc == 0 && num_data > 0 ) {
            _printNodeData (
                root, data, num_data, native, aFullpath, !native, pb );
        }
    ...
    if ( rc == 0 ) {
        if ( count > 0 && !native ) rc = PrintBuffPrint ( pb, "\n" );
        for ( i = 0; i < count; ++i ) {
            char *fullpath = NULL;
            const char *name = NULL;
            ...
            if ( rc == 0 ) {
                ...
                if ( fullpath == NULL ) {
                    rc = RC (
                        rcKFG, rcStorage, rcAllocating, rcMemory, rcExhausted );
                } else {
                    string_printf (
                        fullpath, bsize, NULL, "%s/%s", aFullpath, name );
                }
            }
            if ( rc == 0 ) {
                if ( !isdigit ( name[0] ) ) {
                    ...
                    if ( dname == NULL ) {
                        rc = RC ( rcKFG, rcStorage, rcAllocating, rcMemory,
                            rcExhausted );
                    } else {
    ...
    if ( count > 0 && !native ) { printIndent ( indent, pb ); }

    if ( rc == 0 && !native ) { rc = PrintBuffPrint ( pb, "</%s>\n", root ); }
...
static rc_t CC KConfigPrintImpl ( const KConfig *self, int indent,
    ...
    uint32_t skipCount, va_list args )
{
...
LIB_EXPORT rc_t CC KConfigPrintDebug ( const KConfig *self, const char *path )
{
    ...
    if ( rc == 0 ) { PrintBuffInit ( &pb, NULL ); }

    if ( rc == 0 ) {
        rc = KConfigPrintImpl ( self, 0, path, true, false, &pb, 0, NULL );
    }

    if ( rc == 0 ) { rc = PrintBuffFlush ( &pb ); }
 ...
 */
static rc_t parse_file ( KConfig *self, const char *path, const char *src )
{
    KFGParseBlock pb;
    KFGScanBlock sb;
    ...
    if ( !KFGScan_yylex_init ( &sb, src ) ) { /* out of memory is the only
                                                 reason we can get false here */
        return RC ( rcKFG, rcMgr, rcParsing, rcMemory, rcExhausted );
    }

    KFG_parse ( &pb, &sb ); /* may have reported parse errors into log, but we
                               should have been able to extract enough data to
                               proceed regardless */
...
LIB_EXPORT rc_t CC KConfigParse (
    KConfig *self, const char *path, const char *src )
{
    if ( self == NULL )
 ...
 */
LIB_EXPORT rc_t CC KConfigLoadFile (
    KConfig *self, const char *path, const KFile *file )
{
    rc_t rc;
            ...
            if ( rc == 0 ) {
                ...
                if ( rc == 0 ) {
                    ...
                    DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
                        ( "KFG: loading file '%s'\n", buff ) );
                    if ( name == NULL ) { /* no dir name */
                        UPDATE_NODES ( "", buff );
                    } else {
                        *name = 0; /* nul-terminate dir name; file name follows
                                      the 0 */
...
static bool KConfigNodePrintPath ( KConfigNode *self, PrintBuff *pb )
{
    if ( self->dad == NULL )
        PrintBuffPrint ( pb, "%S", &self->name );
    else {
        if ( KConfigNodePrintPath ( self->dad, pb ) ) return true;
...
static void _KConfigIniKfgSettings ( const KConfig *self, KfgSettings *ks )
{
    size_t bytes = 0;
    ...
    ks->envrNcbiHome = getenv ( "NCBI_HOME" );
    {
        ...
        if ( value != NULL ) {
            ...
            string_printf ( ks->envrNcbiSettings, sizeof ks->envrNcbiSettings,
    ...
    /* default values */
    {
        ...
        if ( rc != 0 || remaining != 0 ) { return; }
        string_printf ( ks->dfltNcbiHome, sizeof ks->dfltNcbiHome, &bytes,
            "%.*s/.ncbi", num_read, home );
        string_printf ( ks->dfltNcbiSettings, sizeof ks->dfltNcbiSettings,
            &bytes, "%.*s/.ncbi/%s", num_read, home, MAGIC_LEAF_NAME );
    }
}

static bool MayICommitTo ( const KConfig *self, const char *path, size_t size )
{
    if ( !s_disable_user_settings ) {
        ...
        rc_t rc = KConfigRead (
            self, "HOME", 0, home, sizeof home, &num_read, &remaining );
        if ( rc != 0 || remaining != 0 ) { return false; }
        string_printf ( dfltNcbiSettings, sizeof dfltNcbiSettings, &bytes,
            "%.*s/.ncbi/%s", num_read, home, MAGIC_LEAF_NAME );
        return string_cmp ( dfltNcbiSettings, bytes, path, size,
                   sizeof dfltNcbiSettings )
            != 0;
    }
}

static rc_t _KConfigGetNcbiHome (
    const KConfig *self, const KfgSettings *ks, char *buf, size_t size )
{
    size_t num_read = 0;
    size_t remaining = 0;
...
static rc_t _KConfigGetNcbiSettings ( const KConfig *self,
    const KfgSettings *ks, char *buf, size_t size, const char *root )
{
    size_t num_read = 0;
    size_t remaining = 0;
...
static void _KConfigSetNcbiHome (
    KConfig *self, const KfgSettings *ks, const char *ncbi_home )
{
    size_t num_read = 0;
    size_t remaining = 0;
    char buf[PATH_MAX] = "";
    ...
    if ( KConfigRead (
             self, "NCBI_HOME", 0, buf, sizeof buf, &num_read, &remaining )
        == 0 ) {
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: NCBI_HOME='%.*s'\n", num_read, buf ) );
    } else {
...
static void _KConfigSetNcbiSettings (
    KConfig *self, const KfgSettings *ks, const char *ncbi_settings )
{
    size_t num_read = 0;
    size_t remaining = 0;
    char buf[PATH_MAX] = "";
    ...
    if ( KConfigRead (
             self, "NCBI_SETTINGS", 0, buf, sizeof buf, &num_read, &remaining )
        == 0 ) {
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: NCBI_SETTINGS='%.*s'\n", num_read, buf ) );
    } else {
...
static bool CC WriteDirtyNode ( BSTNode *n, void *data )
{
    KConfigNode *self = (KConfigNode *)n;
    PrintBuff *pb = data;
    ...
    if ( self->dirty
        || ( self->came_from != NULL && self->came_from->is_magic_file
               && !self->internal ) ) {
        if ( KConfigNodePrintPath ( self, pb ) ) return true;

        if ( PrintBuffPrint ( pb, " = " ) != 0 ) return true;

        if ( PrintBuffPrintQuoted ( pb, &self->value ) != 0 ) return true;
...
static void CC UnsetDirtyNode ( BSTNode *n, void *data )
{
    KConfigNode *self = (KConfigNode *)n;
...
static rc_t path_to_magic_file (
    const KConfig *self, char *path, size_t buffer_size, size_t *path_size )
{
    const KConfigNode *node;
    ...
    if ( rc == 0 ) {
        ...
        rc = KConfigNodeRead (
            node, 0, path, buffer_size - 1, &num_read, &remaining );
...
LIB_EXPORT rc_t CC KConfigCommit ( KConfig *self )
{
    ...
    rc = path_to_magic_file (
        self, magic_file_path, sizeof magic_file_path, &path_size );
    if ( rc == 0 ) {
        char tmp_file_path[4096];

        if ( !MayICommitTo ( self, magic_file_path, path_size ) ) {
            DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
                ( "KFG: User Settings Are Disables: Skipped "
                  "KConfigCommit\n" ) );
        ...
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: Committing to '%s'\n", magic_file_path ) );
        rc = string_printf ( tmp_file_path, sizeof tmp_file_path, NULL,
            "%s.tmp", magic_file_path );
        if ( rc == 0 ) {
            KDirectory *dir;
            ...
            if ( rc == 0 ) {
                ...
                rc = KDirectoryCreateFile ( dir, &tmp, false, 0600,
                    kcmInit | kcmParents, "%s", tmp_file_path );
                ...
                if ( rc == 0 ) {
                    ...
                    if ( rc == 0 ) {
                        /* flush nodes to file */
                        if ( BSTreeDoUntil (
                                 &self->tree, false, WriteDirtyNode, &pb ) )
                            /* failure */
                            rc = pb.rc;
                        else
                    ...
                    if ( rc == 0 ) {
                        /* Rename file */
                        rc = KDirectoryRename (
                            dir, true, tmp_file_path, magic_file_path );
                        if ( rc == 0 ) {
                            /* clear dirty flags */
                            BSTreeForEach (
                                &self->tree, false, UnsetDirtyNode, &pb );
...
static rc_t record_magic_path (
    KConfig *self, const KDirectory *dir, const char *path, size_t sz )
{
    char buff[4096];
    rc_t rc = KDirectoryResolvePath (
        dir, true, buff, sizeof buff, "%.*s", (int)sz, path );
    if ( rc == 0 ) {
...
static rc_t make_include_path ( KConfigIncluded **p, const KDirectory *dir,
    const char *path, size_t sz, bool is_magic )
{
    char buff[4096];
    rc_t rc = KDirectoryResolvePath (
        dir, true, buff, sizeof buff, "%.*s", (int)sz, path );
...
static bool load_from_file_path ( KConfig *self, const KDirectory *dir,
    const char *path, size_t sz, bool is_magic )
{
    rc_t rc;
    ...
    if ( is_magic ) record_magic_path ( self, dir, path, sz );

    DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
        ( "KFG: try to load from file '%.*s'\n", (int)sz, path ) );
    ...
    if ( rc == 0 ) {
        ...
        if ( rc == 0 ) {
            BSTNode *existing;
            if ( BSTreeInsertUnique ( &self->included, &include->n, &existing,
                     KConfigIncludedSort )
                != 0 )
                free ( include );
            else {
...
static rc_t CC scan_config_path (
    const KDirectory *dir, uint32_t type, const char *name, void *data )
{
    ...
    case kptFile | kptAlias: {
        ...
        if ( sz >= 5 && strcase_cmp ( &name[sz - 4], 4, ".kfg", 4, 4 ) == 0 )
            pb->loaded
                |= load_from_file_path ( pb->self, dir, name, sz, false );
...
static bool scan_config_dir ( KConfig *self, const KDirectory *dir )
{
...
static bool load_from_dir_path (
    KConfig *self, const KDirectory *dir, const char *path, size_t sz )
{
    bool loaded = false;
    const KDirectory *cfg_dir;
    rc_t rc = KDirectoryOpenDirRead (
        dir, &cfg_dir, false, "%.*s", (uint32_t)sz, path );
    if ( rc == 0 ) {
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: try to load from directory '%.*s'\n", (int)sz, path ) );
    ...
    KConfig *self, const KDirectory *dir, const char *path, size_t sz )
{
    ...
    if ( naughty == NULL && sz != 0 ) {
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: try to load from path '%.*s'\n", (int)sz, path ) );
        switch (
            KDirectoryPathType ( dir, "%.*s", (int)sz, path ) & ~kptAlias ) {
        case kptFile:
            loaded = load_from_file_path ( self, dir, path, sz, false );
            break;
        case kptDir: loaded = load_from_dir_path ( self, dir, path, sz ); break;
        }
    }
    return loaded;
}

static bool load_from_path_list (
    KConfig *self, const KDirectory *dir, const char *path )
{
    ...
    while ( path < end ) {
        ...
        if ( sep == NULL ) sep = end;
        if ( load_from_path ( self, dir, path, sep - path ) ) loaded = true;
        path = sep + 1;
    }
    return loaded;
}

static bool load_from_env_variable ( KConfig *self, const KDirectory *dir )
{
    const char *env_list[] = {"KLIB_CONFIG", "VDB_CONFIG", "VDBCONFIG"};
    ...
    for ( i = 0; !loaded && i < sizeof env_list / sizeof env_list[0]; ++i ) {
        ...
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: try to load from env. var '%s'\n", env_list[i] ) );
...
static rc_t load_env_variable_rc (
    KConfig *self, const KDirectory *dir, bool *loaded )
{
    const char *env_list[] = {"NCBI_VDB_CONFIG"};
    ...
    *loaded = false;
    ...
    for ( i = 0; !*loaded && i < sizeof env_list / sizeof env_list[0]; ++i ) {
        ...
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: try to load from env. var '%s'\n", env_list[i] ) );
        if ( eval == NULL || eval[0] == 0 ) return 0;
...
static bool load_from_std_location ( KConfig *self, const KDirectory *dir )
{
    const char *std_locs[] = {
#if !WINDOWS
        "/etc/ncbi",
    ...
    const char *NCBI_VDB_NO_ETC_NCBI_KFG
        = getenv ( "NCBI_VDB_NO_ETC_NCBI_KFG" );
    ...
    for ( i = 0; !loaded && i < sizeof std_locs / sizeof std_locs[0]; ++i ) {
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: try to load from std. location '%s'\n", std_locs[i] ) );
        /* rc = */ KConfigAppendToLoadPath ( self, std_locs[i] );
        loaded = load_from_path (
            self, dir, std_locs[i], string_size ( std_locs[i] ) );
    }
    if ( loaded )
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: found from std. location\n" ) );
    return loaded;
}

static bool load_from_fs_location ( KConfig *self, const char *confdir )
{
    bool loaded = false;
    KDyld *dyld;
    ...
    if ( rc == 0 ) {
        ...
        if ( rc == 0 ) {
            ...
            if ( KDirectoryResolvePath (
                     dir, true, resolved, sizeof resolved, confdir )
                == 0 ) {
...
{
    DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
        ( "KFG: loading from default string\n" ) );
    return parse_file (
               self, "enbedded default configuration string", default_kfg )
        == 0;
}

LIB_EXPORT rc_t CC KConfigGetLoadPath ( const KConfig *self, const char **path )
{
    if ( self == NULL ) {
...
static bool load_user_settings (
    KConfig *self, const KDirectory *dir, const char *path )
{
...
static bool load_from_home ( KConfig *self, const KDirectory *dir,
    const KfgSettings *ks, char *ncbi_settings, size_t ncbi_settings_size )
{
    ...
    if ( _KConfigGetNcbiHome ( self, ks, ncbi_home, sizeof ncbi_home ) != 0 ) {
        ...
        bool loaded = load_from_path (
            self, dir, ncbi_home, string_size ( ncbi_home ) );
        ...
        if ( load_user_settings ( self, dir, ncbi_settings ) ) loaded = true;
...
static rc_t load_config_files ( KConfig *self, const KDirectory *dir,
    const KfgSettings *ks, bool *loaded_from_dir )
{
    rc_t rc;
    bool loaded;
    KDirectory *wd;
    ...
    *loaded_from_dir = false;
    ...
    if ( dir != NULL ) {
        ...
        if ( loaded )
            DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
                ( "KFG: found from supplied directory\n" ) );
    ...
    if ( !loaded ) loaded = load_from_std_location ( self, wd );

    /* check for config as the result of a user install
       i.e. not an admin installation */
    if ( !loaded ) loaded = load_from_fs_location ( self, "../etc/ncbi" );
    if ( !loaded ) loaded = load_from_fs_location ( self, "ncbi" );

    if ( !loaded ) loaded = load_from_default_string ( self );
    ...
    if ( !s_disable_user_settings )
        ...
        DBGMSG ( DBG_KFG, DBG_FLAG ( DBG_KFG ),
            ( "KFG: User Settings Are Disables: Skipped Load KFG From "
              "Home\n" ) );
    ...
    if ( self->load_path ) {
        ...
        if ( tmp ) { self->load_path = tmp; }
    }

    return 0;
}

static void add_predefined_nodes ( KConfig *self, const char *appname )
{
    size_t bytes;
    char buf[4096];
    const char *value;
...
#undef BUILD
    ...
    if ( appname != NULL && rc == 0 ) {
        ...
        if ( value == NULL ) value = string_rchr ( appname, bytes, '\\' );
        if ( value != NULL ) bytes = appname + bytes - value;
        rc = KDirectoryResolvePath (
            cwd, true, buf, sizeof buf, "%s", appname );
        if ( rc == 0 && bytes != 0 ) buf[string_size ( buf ) - bytes] = 0;
    ...
    /* some of the variables may be undefined, create nodes with empty values
     * for them */
...
#endif
    if ( value == NULL ) { value = getenv ( "HOME" ); }

    if ( value == NULL ) {
        update_node ( self, "HOME", "", true );
    } else {
        rc = KDirectoryResolvePath (
            cwd, true, buf, sizeof ( buf ), "%s", value );
        if ( rc == 0 ) {
...
{
    if ( b == NULL || sz < 3 ) { return false; }

    return ( b[0] == '/' && isletter ( b[1] ) && b[2] == '/' );
}

static bool _DetectRepeatedDrives (
    const char *buffer, size_t bsize, size_t *fixed, const KDirectory *dir )
{
    assert ( fixed );

    if ( !sls ( buffer, bsize ) ) { return false; }

    if ( !sls ( buffer + 2, bsize - 2 ) ) { return false; }

    if ( isexistingdir ( buffer, dir ) ) { return false; }

    *fixed = 2;

    while ( bsize > *fixed + 2 ) {
        if ( isexistingdir ( buffer + *fixed, dir ) ) { break; }

        if ( !sls ( buffer + *fixed + 2, bsize - *fixed - 2 ) ) { break; }
...
static rc_t _KConfigNodeFixRepeatedDrives (
    KConfigNode *self, bool *updated, const KDirectory *dir )
{
    rc_t rc = 0;
    ...
    rc = KConfigNodeRead (
        self, 0, buffer, sizeof buffer, &num_read, &remaining );
    if ( rc == 0 ) {
        size_t fixed = 0;
        if ( num_read < sizeof buffer ) { buffer[num_read] = '\0'; }
...
static rc_t _KConfigFixRepeatedDrives (
    KConfig *self, const KDirectory *pdir, bool *updated )
{
    rc_t rc = 0;
    KDirectory *dir = (KDirectory *)pdir;
    KConfigNode *user = NULL;
    if ( dir == NULL ) { rc = KDirectoryNativeDir ( &dir ); }
    rc = KConfigOpenNodeUpdate ( self, &user, "/repository/user" );
    if ( rc == 0 ) {
        _KConfigNodeFixChildRepeatedDrives (
            user, dir, updated, "default-path" );
    }
    if ( rc == 0 ) {
        uint32_t i = 0;
        uint32_t count = 0;
        ...
        for ( i = 0; rc == 0 && i < count; ++i ) {
            ...
            if ( rc == 0 ) { /* main protected ... */
                ...
                rc_t rc
                    = KConfigNodeOpenNodeUpdate ( user, &category, nCategory );
                if ( rc == 0 ) {
                    uint32_t i = 0;
                    uint32_t count = 0;
                    KNamelist *subcategories = NULL;
                    rc_t rc
                        = KConfigNodeListChildren ( category, &subcategories );
                    if ( rc == 0 ) { /* main protected ... */
                    ...
                    for ( i = 0; rc == 0 && i < count; ++i ) {
                        ...
                        if ( rc == 0 ) {
                            _KConfigNodeFixChildRepeatedDrives (
                                category, dir, updated, "%s/%s", name, "root" );
                        }
                    }
    ...
    if ( pdir == NULL ) { RELEASE ( KDirectory, dir ); }
    return rc;
}
...
static rc_t _KConfigUpdateDefault ( KConfig *self, bool *updated,
    const char *node_name, const char *node2_name, const char *old_value,
    const char *new_value, const char *updated_name )
{
    rc_t rc = 0;
    ...
    *updated = false;
    ...
    if ( rc == 0 ) { /* when found: update just value = old-value */
        if ( StringEqual ( &sOldValue, result ) ) update1 = *updated = true;
        free ( result );
        result = NULL;
    ...
    if ( node2_name != NULL ) {
        ...
        if ( rc == 0 ) { /* when found: update just value = old-value */
            if ( StringEqual ( &sOldValue, result ) ) update2 = *updated = true;
            free ( result );
            result = NULL;
        } else /* don't update when node did not exist */
            rc = 0;
    ...
    if ( *updated ) {
        assert ( rc == 0 );
        if ( update1 ) rc = KConfigWriteString ( self, node_name, new_value );
        if ( rc == 0 && update2 )
...
{
    return _KConfigUpdateDefault ( self, updated, "/tools/ascp/max_rate", NULL,
        "1000m", "450m", "/tools/ascp/max_rate/450m" );
}
...
static rc_t KConfigFill (
    KConfig *self, const KDirectory *cfgdir, const char *appname, bool local )
{
    KConfigNode *root;
    String empty;
    rc_t rc;
...
static rc_t KConfigMakeImpl ( KConfig **cfg, const KDirectory *cfgdir,
    bool local, bool disableFileLoadOnKfgCreating )
{
    rc_t rc;
    ...
    if ( cfg == NULL )
        ...
        if ( mgr == NULL )
            ...
            if ( rc == 0 ) {
                ...
                if ( rc == 0 && updated ) rc = KConfigCommit ( mgr );
#endif
            }
            ...
            if ( rc == 0 ) {
                if ( !local ) { atomic_test_and_set_ptr ( &G_kfg, mgr, NULL ); }
                *cfg = mgr;
                return 0;
            }
...
/* Whack
 */
static rc_t CC KfgConfigNamelistWhack ( KfgConfigNamelist *self )
{
    free ( self );
    return 0;
}
...
/* Count
 */
static rc_t CC KfgConfigNamelistCount (
    const KfgConfigNamelist *self, uint32_t *count )
{
...
/* Get
 */
static rc_t CC KfgConfigNamelistGet (
    const KfgConfigNamelist *self, uint32_t idx, const char **name )
{
...
static KNamelist_vt_v1 vtKfgConfigNamelist = {
    ...
    KfgConfigNamelistWhack, KfgConfigNamelistCount, KfgConfigNamelistGet
    /* end minor version 0 methods */
};

static rc_t KfgConfigNamelistMake ( KNamelist **names, uint32_t count )
{
    rc_t rc;
    KfgConfigNamelist *self = malloc ( sizeof *self - sizeof self->namelist
        + count * sizeof self->namelist[0] );
    if ( self == NULL )
        ...
        rc = KNamelistInit (
            &self->dad, (const KNamelist_vt *)&vtKfgConfigNamelist );
        if ( rc == 0 ) {
            *names = &self->dad;
            return 0;
        }
 ...
 */
static void CC BSTNodeCount ( BSTNode *n, void *data )
{
    *(uint32_t *)data += 1;
}

static void CC KConfigNodeGrabName ( BSTNode *n, void *data )
{
    KfgConfigNamelist *list = data;
    list->namelist[list->count++] = ( (const KConfigNode *)n )->name.addr;
}

/* ListChildren
 *  list all named children
 */
LIB_EXPORT rc_t CC KConfigNodeListChildren (
    const KConfigNode *self, KNamelist **names )
{
    if ( names == NULL )
    ...
    *names = NULL;
    ...
    if ( self != NULL ) {
        ...
        if ( rc == 0 )
            BSTreeForEach ( &self->children, 0, KConfigNodeGrabName, *names );
...
static void CC KConfigGrabName ( BSTNode *n, void *data )
{
    KfgConfigNamelist *list = data;
    list->namelist[list->count++] = ( (const KConfigIncluded *)n )->path;
}

/* ListIncluded
 *  list all included files
 */
LIB_EXPORT rc_t CC KConfigListIncluded (
    const KConfig *self, KNamelist **names )
{
    if ( names == NULL ) return RC ( rcKFG, rcMgr, rcListing, rcParam, rcNull );
    ...
    *names = NULL;
    ...
    if ( self != NULL ) {
        ...
        rc = KfgConfigNamelistMake ( names, count );
        if ( rc == 0 )
            BSTreeForEach ( &self->included, 0, KConfigGrabName, *names );
 ...
 */
static rc_t ReadNodeValueFixed (
    const KConfigNode *self, char *buf, size_t bsize )
{
    size_t num_read, remaining;
    rc_t rc
        = KConfigNodeRead ( self, 0, buf, bsize - 1, &num_read, &remaining );
    if ( rc == 0 ) {
        if ( remaining != 0 )
            rc = RC ( rcKFG, rcNode, rcReading, rcBuffer, rcInsufficient );
        else
 ...
 * result [ OUT ] - return value (true if "TRUE", false if "FALSE"; rc != 0 if
 * neither)
 *
 */
        ...
        *result = false;
        ...
        if ( self == NULL )
            ...
            if ( rc == 0 ) {
                ...
                case 'n':
                    if ( buf[1] == 0 ) return 0;
                    if ( strncasecmp ( buf, "no", sizeof buf ) == 0 ) return 0;
                    break;
                ...
                case 'y':
                    *result = true;
                    if ( buf[1] == 0 ) return 0;
                    if ( strncasecmp ( buf, "yes", sizeof buf ) == 0 ) return 0;
                    break;
                }
 ...
 *
 */
LIB_EXPORT rc_t CC KConfigNodeReadI64 (
    const KConfigNode *self, int64_t *result )
{
    rc_t rc;
 ...
 *
 */
LIB_EXPORT rc_t CC KConfigNodeReadU64 (
    const KConfigNode *self, uint64_t *result )
{
    rc_t rc;
 ...
 *
 */
LIB_EXPORT rc_t CC KConfigNodeReadF64 (
    const KConfigNode *self, double *result )
{
    rc_t rc;
 ...
 *
 */
LIB_EXPORT rc_t CC KConfigNodeReadString (
    const KConfigNode *self, String **result )
{
    rc_t rc;
        ...
        if ( self == NULL )
            ...
            if ( rc == 0 ) {
                ...
                if ( value == NULL )
                    ...
                    StringInit ( value, (char *)( value + 1 ), to_read,
                        (uint32_t)to_read );

                    /* read the actual data */
                    rc = ReadNodeValueFixed (
                        self, (char *)value->addr, to_read + 1 );
                    if ( rc == 0 ) {
                        ...
                        *result = value;
                        ...
                        rc = RC (
                            rcKFG, rcNode, rcReading, rcFormat, rcIncorrect );
                        free ( value );
                    }
                }
            }
...
LIB_EXPORT rc_t CC KConfigRead ( const KConfig *self, const char *path,
    size_t offset, char *buffer, size_t bsize, size_t *num_read,
    size_t *remaining )
{
    const KConfigNode *node = NULL;
...
LIB_EXPORT rc_t CC KConfigReadBool (
    const KConfig *self, const char *path, bool *result )
{
    NODE_TO_CONFIG_ACCESSOR ( KConfigNodeReadBool );
}
LIB_EXPORT rc_t CC KConfigReadI64 (
    const KConfig *self, const char *path, int64_t *result )
{
    NODE_TO_CONFIG_ACCESSOR ( KConfigNodeReadI64 );
}
LIB_EXPORT rc_t CC KConfigReadU64 (
    const KConfig *self, const char *path, uint64_t *result )
{
    NODE_TO_CONFIG_ACCESSOR ( KConfigNodeReadU64 );
}

LIB_EXPORT rc_t CC KConfigReadF64 (
    const KConfig *self, const char *path, double *result )
{
    NODE_TO_CONFIG_ACCESSOR ( KConfigNodeReadF64 );
}

LIB_EXPORT rc_t CC KConfigReadString (
    const KConfig *self, const char *path, struct String **result )
{
    NODE_TO_CONFIG_ACCESSOR ( KConfigNodeReadString );
}

#define DISP_RC( rc, msg )                                                     \
    (void)( ( rc == 0 ) ? 0 : LOGERR ( klogInt, rc, msg ) )
...
static rc_t open_file ( const KFile **f, const char *path )
{
    /* get current directory */
    KDirectory *wd;
...
/* KFS_EXTERN rc_t CC KFileMakeGzip2ForRead ( struct KFile const **gz, struct
KFile const *src ); #include <stdio.h> aprintf */
#define aprintf( a, b ) ( (void)0 )
static rc_t decode_ncbi_gap ( KDataBuffer *mem, const KFile *orig )
{
    char hdr[8];
    size_t num_read;
    ...
    if ( rc == 0 && num_read == sizeof hdr ) {
        if ( memcmp ( hdr, "ncbi_gap", sizeof hdr ) != 0 ) {
            ...
            if ( rc == 0 ) {
                ...
                aprintf ( "decode_ncbi_gap %d\n", __LINE__ );
                rc = KFileMakeSubRead (
                    &sub, orig, sizeof hdr, eof - sizeof hdr );
                aprintf ( "decode_ncbi_gap %d\n", __LINE__ );
                if ( rc == 0 ) {
                    const KFile *gzip;
                    aprintf ( "decode_ncbi_gap %d\n", __LINE__ );
                    ...
                    if ( rc == 0 ) {
                        ...
                        if ( rc == 0 ) {
                            ...
                            for ( total = 0;; ) {
                                ...
                                rc = KFileReadAll ( gzip, total, &buff[total],
                                    to_read, &num_read );
                                aprintf ( "decode_ncbi_gap %d\n", __LINE__ );
                                if ( rc != 0 ) break;
...
static rc_t _KConfigNncToKGapConfig (
    const KConfig *self, char *text, KGapConfig *kgc )
{
    size_t len = 0;
    int i = 0;
    ...
    for ( i = 0;; ++i ) {
        ...
        if ( p == NULL ) { break; }
        switch ( i ) {
        case 0: kgc->projectId = p; break;
        case 1: kgc->encryptionKey = p; break;
        case 2: kgc->downloadTicket = p; break;
        case 3: kgc->description = p; break;
        }
    }
...
LIB_EXPORT rc_t KConfigFixMainResolverCgiNode ( KConfig *self )
{
    ...
    if ( rc == 0 ) {
        rc = KConfigOpenNodeUpdate (
            self, &node, "/repository/remote/main/CGI/resolver-cgi" );
    }

    if ( rc == 0 ) { rc = KConfigNodeReadString ( node, &result ); }

    if ( rc == 0 ) {
        String http;
        CONST_STRING (
            &http, "http://www.ncbi.nlm.nih.gov/Traces/names/names.cgi" );
        assert ( result );
...
LIB_EXPORT rc_t KConfigFixProtectedResolverCgiNode ( KConfig *self )
{
    ...
    if ( rc == 0 ) {
        rc = KConfigOpenNodeUpdate (
            self, &node, "/repository/remote/protected/CGI/resolver-cgi" );
    }
...
static rc_t _KConfigNodeUpdateChild (
    KConfigNode *self, const char *name, const char *val )
{
    rc_t rc = 0;
    ...
    if ( rc == 0 ) { rc = KConfigNodeWrite ( node, val, string_size ( val ) ); }
...
static rc_t _KConfigMkPwdFileAndNode (
    KConfig *self, KConfigNode *rep, const KGapConfig *kgc )
{
    rc_t rc = 0;
    ...
    if ( rc == 0 ) { rc = KConfigNodeReadString ( ncbiHome, &result ); }
    ...
    if ( rc == 0 ) {
        ...
        if ( rc == 0 ) { assert ( num_writ < sizeof encryptionKeyPath ); }
    }
    free ( result );
    result = NULL;

    if ( rc == 0 ) { rc = KDirectoryNativeDir ( &dir ); }

    if ( rc == 0 ) {
        rc = KDirectoryCreateFile ( dir, &encryptionKeyFile, false, 0600,
            kcmInit | kcmParents, "%s", encryptionKeyPath );
    }
    ...
    if ( rc == 0 ) {
        ...
        rc = KFileWrite ( encryptionKeyFile, 0, kgc->encryptionKey,
            string_size ( kgc->encryptionKey ), &num_writ );
        if ( rc == 0 ) {
    ...
    if ( rc == 0 ) {
        rc = _KConfigNodeUpdateChild (
            rep, "encryption-key-path", encryptionKeyPath );
    }
    ...
    if ( rc == 0 ) {
        ...
        if ( type == kptNotFound ) {
            rc = KDirectoryCreateDir (
                wd, 0777, kcmCreate | kcmParents, "%s", repoParentPath );
        }
    }
...
static rc_t _KConfigDBGapRepositoryNodes ( KConfig *self, KConfigNode *rep,
    ...
    const char **newRepoParentPath )
{
    ...
    if ( rc == 0 ) { rc = _KConfigMkPwdFileAndNode ( self, rep, kgc ); }

    if ( rc == 0 ) {
        assert ( kgc->downloadTicket );
        rc = _KConfigNodeUpdateChild (
            rep, "download-ticket", kgc->downloadTicket );
    }
    ...
    if ( rc == 0 ) {
        ...
        if ( repoParentPath == NULL ) {
            ...
            if ( rc == 0 ) { rc = KConfigOpenNodeRead ( self, &home, "HOME" ); }

            if ( rc == 0 ) { rc = KConfigNodeReadString ( home, &result ); }
            ...
            if ( rc == 0 ) { repoParentPath = rootPath; }
        ...
        if ( rc == 0 ) { rc = _mkNotFoundDir ( repoParentPath ); }
...
static rc_t _KConfigAddDBGapRepository ( KConfig *self, const KGapConfig *kgc,
    const char *repoParentPath, const char **newRepoParentPath )
{
    rc_t rc = 0;
    ...
    if ( rc == 0 ) {
        ...
        if ( rc == 0 ) { assert ( num_writ < sizeof repNodeName ); }
    }
    ...
    if ( rc == 0 ) {
        rc = _KConfigDBGapRepositoryNodes (
            self, rep, kgc, repoParentPath, newRepoParentPath );
    }
...
LIB_EXPORT rc_t CC KConfigImportNgc ( KConfig *self, const char *ngcPath,
    const char *repoParentPath, const char **newRepoParentPath )
{
    if ( self == NULL ) {
    ...
    if ( ngcPath == NULL ) {
        ...
        if ( rc != 0 ) {
            ...
            /*DBGMSG(DBG_KFG, DBG_FLAG(DBG_KFG), ("KConfigImportNgc %d\n",
            __LINE__)); aprintf("KConfigImportNgc %d\n", __LINE__); */
            ...
            if ( rc == 0 ) {
                /*DBGMSG(DBG_KFG, DBG_FLAG(DBG_KFG), ("KConfigImportNgc %d\n",
                __LINE__)); aprintf("KConfigImportNgc %d\n", __LINE__);*/
                rc = _KConfigAddDBGapRepository (
                    self, &kgc, repoParentPath, newRepoParentPath );
                /*DBGMSG(DBG_KFG, DBG_FLAG(DBG_KFG), ("KConfigImportNgc %d\n",
                __LINE__)); aprintf("KConfigImportNgc %d\n", __LINE__);*/
            }
...
LIB_EXPORT rc_t CC KConfigWriteBool (
    KConfig *self, const char *path, bool value )
{
    KConfigNode *node;
...
LIB_EXPORT rc_t CC KConfigWriteString (
    KConfig *self, const char *path, const char *value )
{
    KConfigNode *node;
    rc_t rc = KConfigOpenNodeUpdate ( self, &node, "%s", path );
    if ( rc == 0 ) {
...
LIB_EXPORT rc_t CC KConfigWriteSString (
    KConfig *self, const char *path, struct String const *value )
{
    KConfigNode *node;
    rc_t rc = KConfigOpenNodeUpdate ( self, &node, "%s", path );
    if ( rc == 0 ) {

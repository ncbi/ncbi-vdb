...
typedef struct KRU64Index_PNode KRU64Index_PNode;
struct KRU64Index_PNode
{
    uint64_t key;
    uint64_t key_size;
    int64_t id;
 ...
 */
static
bool CC KRU64Index_Grep(PBSTNode *node, void *data)
{
    const KRU64Index_PNode* n = node->data.addr;
    KRU64Index_GrepData* d = data;
...
{
    KRU64Index_GrepData d;

    memset(&d, 0, sizeof(KRU64Index_GrepData));
    d.search.key = offset;
    d.key = key;
    d.key_size = key_size;
    d.id = id;
    d.id_qty = id_qty;
    if( !PBSTreeDoUntil(self->tree, false, KRU64Index_Grep, &d) ) {
    ...
    rc_t (CC*f)(uint64_t key, uint64_t key_size, int64_t id, uint64_t id_qty, void* data), void* data)
{
    ...
    d.data = data;
    d.search.key = offset;
    PBSTreeDoUntil(self->tree, false, KRU64Index_Grep, &d);
    return d.rc;
}

	...
	if(data_offset != NULL || data_length != NULL || repeat_count != NULL){
		if(!cself->expanded){
			...
			default: /*** should not happen to anything but 2 above **/
				return RC (rcDB, rcPagemap, rcSearching, rcData, rcInconsistent );
		}
	}
	return 0;
}
...
rc_t PageMapFindRow(const PageMap *cself,uint64_t row,uint32_t * data_offset,uint32_t * data_length,uint32_t * repeat_count)
{
	...
	if(cself->random_access && cself->leng_recs == 1){
		if(row >= cself->data_recs)
			return RC(rcDB, rcPagemap, rcAccessing, rcRow, rcOutofrange);
		if(data_length)  *data_length = cself->length[0];
...
rc_t PageMapGrow(PageMap *self, uint32_t new_reserve_leng, uint32_t new_reserve_data) {
    ...
    if (new_reserve_leng >= (1UL << 31) || new_reserve_data >= (1UL << 31))
        return RC(rcDB, rcPagemap, rcAllocating, rcParam, rcExcessive);
...
rc_t PageMapNew(PageMap **lhs, uint32_t reserve) {
    ...
    if (y == NULL)
        return RC(rcDB, rcPagemap, rcConstructing, rcMemory, rcExhausted);
                ...
                case Z_OK:
                    for (; ;) {
                        ...
                        if (zr == Z_OK) {
                            ...
                            rc = RC(rcDB, rcPagemap, rcWriting, rcParam, rcInvalid);
                            break;
                        }
                    }
                    deflateEnd(&zs);
                    break;
                case Z_MEM_ERROR:
                    rc = RC(rcDB, rcPagemap, rcWriting, rcMemory, rcExhausted);
                    break;
                default:
                    rc = RC(rcDB, rcPagemap, rcWriting, rcParam, rcInvalid);
                }
...
rc_t PageMapDeserialize_v0(PageMap **lhs, uint8_t const src[], unsigned ssize, row_count_t row_count) {
	...
	rc = deserialize_lengths((**lhs).length, (unsigned)(2*leng_recs+data_recs), &src[cur], ssize - cur, &sz);
        ...
        break;
     default:
	rc = RC(rcDB, rcPagemap, rcConstructing, rcFormat, rcUnrecognized);
    }
...
rc_t PageMapDeserialize_v1(PageMap **lhs, const uint8_t *Src, uint64_t ssize, uint32_t row_count) {
    ...
    case 3:
        ...
        break;
    default:
        return RC(rcDB, rcPagemap, rcConstructing, rcData, rcInvalid);
    }
    if (rc)
        return rc;
    ...
    if (zr == Z_OK) {
        ...
        if (zr != Z_STREAM_END)
            rc = RC(rcDB, rcPagemap, rcConstructing, rcData, rcInvalid);
        inflateEnd(&zs);
    }
    else
        rc = RC(rcDB, rcPagemap, rcConstructing, rcMemory, rcExhausted);
...
rc_t PageMapDeserialize (PageMap **lhs, const void *src, uint64_t ssize, uint64_t row_count) {
    ...
    if (lhs == NULL)
        return RC(rcDB, rcPagemap, rcConstructing, rcParam, rcNull);
    ...
    *lhs = NULL;
    ...
    case 2:
        ...
        break;
    default:
        return RC(rcDB, rcPagemap, rcConstructing, rcData, rcBadVersion);
    }
    if (rc == 0)
...
LIB_EXPORT  elem_count_t CC PageMapIteratorDataLength_Ext(const PageMapIterator *self)
{
    return PageMapIteratorDataLength( self );
}

LIB_EXPORT bool CC PageMapIteratorNext_Ext(PageMapIterator *self)
{
    return PageMapIteratorNext( self );
}

LIB_EXPORT elem_count_t CC PageMapIteratorDataOffset_Ext(const PageMapIterator *self)
{
    return PageMapIteratorDataOffset( self );
}

LIB_EXPORT row_count_t CC PageMapIteratorRepeatCount_Ext(const PageMapIterator *cself)
{
